...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve` and `Promise.reject`, `new Promise(resolve => ...)`, typed executor `reject` callbacks, and pending executors that ignore `resolve`, same-type `Promise.all` and `Promise.race`, fulfillment-only `.then()`, awaited console arguments, binary expressions, record fields, and array-intrinsic arguments whose receivers and earlier arguments stay live across suspension, rejection-aware `try/catch/finally` segments with awaited cleanup and completion replacement, non-Promise await hops, captured bindings, and timer- and Immediate-backed promises in straight-line async functions resume through the runtime microtask queue, propagate typed results and rejections, and preserve first-settlement-wins behavior. Unhandled rejections are reported only after the microtask checkpoint and exit nonzero after final safe-Rust heap cleanup; more complex suspension shapes still refuse explicitly.
