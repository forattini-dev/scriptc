...
- **The Rust backend now handles synchronous text filesystem workflows.** UTF-8 reads, writes, appends, existence checks, directory listing and mutation, recursive removal, temporary directories, permissions, copying, renaming, and canonical paths match Node across success, errno propagation, `try/catch/finally`, and typed JSON configuration round-trips.
