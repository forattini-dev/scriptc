...
- **The Rust backend now handles synchronous filesystem workflows.** UTF-8 and byte-exact whole-file and descriptor I/O, positioned reads and writes, appends, existence checks, directory listing and mutation, recursive removal, temporary directories, permissions, copying, renaming, canonical paths, and `statSync`/`lstatSync` metadata match Node across success, errno propagation, `try/catch/finally`, and typed JSON configuration round-trips.
