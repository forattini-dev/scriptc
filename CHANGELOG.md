...
- **Synchronous child commands now run through the safe-Rust backend.** `execFileSync` and shell-backed `execSync` support UTF-8 capture, stdin input, cwd and environment selection, pipe/ignore/inherit stdio, timeouts, status failures, and spawn errors with catchable Node-compatible messages and codes.
