...
- **Module cycles the Effect style writes are admitted.** The benign-cycle admission's inert-top-level rule (`frontend/cycle-inert.ts`) now sees through member calls on declaration-owned values (`Schema.Struct({…}).annotate(…)`, `RelativePath.pipe(Schema.optional)`, a branded schema's `.make`), kernel construction that only stores callbacks (`Effect.gen`, `Layer.effect`, `Effect.fn`; `run*`/`unsafe*` stay refused), module-namespace member reads, calls into program functions whose module cannot reach the cycle's cluster, hoisted function declarations with inert bodies (`define(p) { return p }`), and imports of function declarations across the cycle-closing edge (hoisted at instantiation, never a TDZ). `SCRIPTC_CYCLE_DEBUG=1` lists every offender of a cluster. A `--npm-static` WORKSPACE package importing `node:module` keeps the per-statement fence instead of demoting the whole package (the bundler-banner rule is for bundles).
- **The effect kernel, first slice: `Effect` values compile to native Rust with no engine.** In a static build, `import { Effect } from "effect"` binds the package's namespace to the compiler's own lowering (`frontend/lowering/lower-effect.ts`, matched by the binding's provenance — the module namespace of `effect/dist/Effect.d.ts` — so a user object named `Effect` never matches), and `Effect<A, E, R>` is an opaque refcounted IR kind the native kernel runs (`runtime/effect.rs`: a description tree; values cross it boxed and are unboxed with the type the checker knows at each site, no dynamic checks). This slice covers `Effect.succeed`, `Effect.sync`, `Effect.fail`, `Effect.die`, `Effect.map`, `Effect.flatMap`, `Effect.catch` (and v3's `catchAll`), `Effect.mapError`, `Effect.orDie`, `Effect.runSync`, `Effect.runPromise` and `Effect.gen`: a generator body is one of the runtime's own generators (fibers), `yield* effect` hands the kernel the effect and reads its value back with the checker's type, a failing `yield*` ends the body, and the kernel's interpreter is an explicit continuation stack (no recursion) with the failure channel threaded through catch/mapError/orDie frames. `Effect.promise` and `Effect.tryPromise({ try, catch })` suspend the fiber on the program's own promise (a rejection is a defect for the former, the typed failure `catch` answers for the latter, with the reason as the program's dynamic value) and resume it from the promise's reaction on a later loop turn; `runSync` across an asynchronous boundary throws Effect's own refusal. `Effect.fn("name")(function* (a, b) { … })` and `Effect.fnUntraced` answer a function value (a runtime-callback closure) whose calls run the body as an effect — effect's `(...args: Args) => Effect` result type maps structurally now that a rest parameter instantiated at a required-only tuple counts as fixed slots; `effect.pipe(step, …)` and effect's own `pipe(effect, step, …)` fold data-last combinators (`Effect.map(f)`, `Effect.catch(f)`, `Effect.andThen(x)`, `Effect.as(v)`, bare `Effect.asVoid`/`ignore`/`orDie`); `Effect.void`, `Effect.as`, `asVoid`, `ignore` and `andThen` (over an effect, a thunk or a function) join. Services and layers: a `class Service extends Context.Service<Self, Shape>()("id") {}` declaration is the kernel's service KEY (an effect that looks the service up in the fiber's environment — `yield* Service`, `Effect.service(Service)`), `Effect.provideService`, `Layer.succeed`/`Layer.effect`/`Layer.provide`/`Layer.provideMerge`/`Layer.merge`/`Layer.mergeAll`/`Layer.empty` describe environments (a layer builds as an effect answering its service bundle; `provide` hides the inner services, `provideMerge` keeps them; no memoization yet, so a layer referenced twice builds twice), and `Effect.provide` installs them; `Layer<…>` values share the effect handle. `Effect.forEach(items, (a, i) => effect, { discard })` and `Effect.all` over a tuple, an array or a record (with `{ discard }`) run sequentially (a `concurrency` option is accepted and ignored) and rebuild the program's typed collection from the kernel's boxed values through a carrier the lowering emits; effects are array elements now (`Effect.all(items.map(f))`). `Effect.log`/`logInfo`/`logWarning`/`logError`/`logDebug`/`logTrace` print effect 4's default logger line (`[HH:MM:SS.mmm] LEVEL (#fiber): parts`, local time, fibers numbered as Effect numbers them, non-string parts through Node's inspect); `tap`/`tapError` observe a channel without changing it, `suspend` defers construction, `withSpan` passes through (spans are not observable natively), `sleep` (milliseconds or `"10 millis"`-style text) suspends the fiber on a timer; scopes: `Effect.scoped`, `addFinalizer` (LIFO, with the scope's exit), `ensuring`, `acquireRelease` and `acquireUseRelease`; `Effect.exit` captures an opaque `Exit` handle (`Exit.isSuccess`/`isFailure`/`succeed`/`fail`, `._tag`, `.value` typed by the checker). `Effect.try({ try, catch })` (the thunk's throw reaches `catch` as the program's dynamic value), `orElseSucceed`, `catchIf` (a typed predicate on the failure); `Option` is an opaque kernel handle — `some`/`none`/`isSome`/`isNone`/`getOrUndefined` (the site's `A | undefined` union)/`getOrElse`/`map`/`match`, `._tag` and `.value` typed by the checker. Kernel-package types are structural in a static build (no engine home to fence to). Every other member is a named refusal that lists what the kernel lacks. Known generator-lane gap: a `yield*` nested inside a method call's argument (hoist it into a `const`). Diagnostics inside a kernel package's own `.d.ts` (effect reaches for DOM and esnext names the fallback surface lacks) no longer gate the build. Schema classes are native classes (`frontend/lowering/lower-schema.ts`): `class X extends Schema.Class<X>("id")({ …fields })`, `Schema.TaggedClass<X>()("tag", { … })`, `Schema.ErrorClass<X>("id")({ … })` and `Schema.TaggedErrorClass<X>()("tag", { … })` collect one field per prop of the fields literal (typed by the checker's instance type, ahead of the body's own fields), `_tag` last, and a constructor taking the props record (`new X({ … })`; an optional prop is a `T | undefined` slot; a propless class takes `new X({})`); the error forms extend the runtime Error (`name` is the tag or identifier, `message` the prop or `""`, so `String(e)`, `instanceof Error` and `catch` work), are yieldable (`yield* new NotFound({ … })` is `Effect.fail`), and `return yield* Effect.fail(…)` in a gen body is the unreachable tail it is in Effect; `Effect.catchTag("Tag", (e) => …)` recovers the failure the tag names, `Effect.runSyncExit` joins, and a failed `runSync`/`runPromise` throws or rejects with the program's error value itself. Instances print like Effect's (`Info { id: 'a', _tag: 'Info' }`, error forms as their toJSON object `{ message, id, _tag }`); optional props holding undefined are omitted, and the props print in declaration order (Effect prints the construction literal's order — the one divergence, with construction-time validation, which the checker already did). Values cross the kernel ARM-WISE now: a union-typed site boxes its arm and unboxes any arm's box back into the union, so a failure typed by one class at `fail` meets a `catch` typed by the union of every failure. `Service.of(impl)` is the identity over the service shape; `Layer.effectDiscard`. **Schema VALUES are native descriptors** (`runtime/schema.rs`, `lower-schema.ts`): `Schema.String/Number/Boolean/Unknown/Any/Finite/Int/Null/Undefined/NumberFromString/Defect()`, `Literal`, `Literals([…])`, `Struct({…})`, `Array`, `Record`, `Union([…])`, `NullOr`, `UndefinedOr`, `optional`, `optionalKey`, `mutable`, `brand` (type-level: `string & Brand<"ID">` is a string), `annotate({ identifier })` (the name effect's messages use), the filters `isStartsWith/isEndsWith/isIncludes/isGreaterThanOrEqualTo/isLessThanOrEqualTo/isGreaterThan/isLessThan/isMinLength/isMaxLength` through `S.check(…)`/`Schema.check(…)`, and the decoders: `decodeUnknownSync(S)` (a function value; a failed decode throws a runtime Error named SchemaError with effect's issue text — `Expected number, got "1"\n  at ["age"]`, `Missing key`, union candidates by literal discriminator, the identifier of an annotated schema), `decodeUnknownOption`, `decodeUnknownEffect` (a `SchemaError` kernel handle in the failure channel, `._tag`/`.message`), `decodeUnknownExit`, `Schema.is(S)(value)`, `encodeSync` (structural schemas), `S.make(props)` (the identity over the checker's `Type`). A schema decodes the program's dynamic value against the descriptor — objects are rebuilt with the declared keys only — and the result converts to the site's static type. A schema DECORATED with program statics (`Schema.String.pipe(Schema.brand("ID"), statics((s) => ({ create: () => s.make(…) })))` — the `Object.assign(schema, methods)` idiom) is a record of the statics plus a hidden schema slot: every schema site unwraps the slot, the statics read as fields, `Object.assign(schema, statics)` builds it, and `S & M` inside a monomorphized body maps the same way; a pipe step that is a one-parameter program function value is called with the source. `Schema.tag("x")` decodes as the required literal and `S.make(props)` fills it (make applies the Struct's constructor defaults over the props as a dynamic value); `Person.make(props)` on a schema class constructs. `Effect.fn("name")(function* …, step, …)` lowers the pipeline steps into a lifted `(effect) => effect` the callback runs over each call's effect. `Duration` is a kernel handle holding milliseconds (`millis/seconds/minutes/hours/days/weeks/zero`, `toMillis/toSeconds`) and `Effect.sleep` accepts it. Every interface/class of effect's runtime modules (Effect, Layer, Cause, Scope, Fiber, Deferred, Queue, Stream, Duration, Config, DateTime, …) is the kernel's opaque handle in a static build, so values of those types flow through records and signatures until a member call names what the kernel lacks. Not yet: transforming schemas (`decodeTo`, `fromJsonString`, `withDecodingDefault`), classes as schemas (`Schema.Array(Person)`), `Schema.is` as a stored predicate value (a generic signature), `catchTags`, `Cause.*`. Corpus 2972 to 2984 (`// @rust-only`, Node oracle running the real package): a 1.4 MB binary, none of effect's JavaScript in it. The census behind this order — effect is 68% of the code a redcode command loads, and redcode's source uses 98 distinct `Effect.*` members — is in the plan.
- **Boot on the V8 island: indexed module tables and a per-binary code cache.** Every import used to walk the whole embedded edge table (a fifth of a redcode command's samples with 4,658 modules); the module and edge tables now carry hash indexes built on first lookup, on both engines. On V8, the emitter stamps the embedded graph with a build id (a digest of every module and edge) and a run persists the code cache of each module it compiled fresh — produced after evaluation, so lazily compiled functions are included — in one file per build id and V8 cache version under the scriptc cache directory; later runs of the binary compile from it, and a module a later command loads first joins the file at teardown or ahead of `process.exit`. V8 rejects a stale or foreign cache and it is regenerated. `SCRIPTC_V8_CODE_CACHE=0` disables it; `SCRIPTC_ISLAND_TRACE=1` prints hits, writes and a boot summary (modules compiled, cache hits, resolve, instantiate and evaluate time). Embedded sources reach V8 as external one-byte strings (no copy into the heap; the inflated text already lives for the process), the four bootstrap scripts are cached the same way, the builtin wrappers have cache entries too (a warm run no longer rewrites the store), and embedded sources inflate through zlib-rs. `SCRIPTC_V8_FLAGS` passes engine flags through for experiments. The require shim's CommonJS wrapper is compiled natively on V8 (`host.compileModule`: the embedded text without a copy, Node's parameter list without the `new Function` line offset, the file's name on the wrapper, and the same code cache — about 630 of redcode's 1,684 boot modules are CommonJS and were re-parsed every run); the C island keeps `new Function`. redcode, best of three: `--version` 0.9 s → 0.5 s, `stats`/`session list` 1.4 s → 0.9 s (Bun: 1.1 s and 1.7 s); a trivial island program boots in 30 ms. The trace summary for `--version`: 1,684 modules from the cache in 50 ms, resolve and inflate 42 ms, instantiate 101 ms, evaluate 280 ms.
- **V8 is the Rust island's engine.** `--dynamic` builds on the Rust backend now embed V8 (rusty_v8 152.2.0, in a sibling crate `scriptc-island-v8` that holds the bridge's unsafe surface; the runtime crate and the program TU stay `forbid(unsafe_code)`) behind the same 53-function island surface generated code already used, so nothing in the emitter changed. The shared JavaScript bootstraps, module tables and every host member (fs, child processes, SQLite, sockets, the http server and client legs, fetch, zlib, timers, crypto) run on both engines; boa stays one flag away (`SCRIPTC_ISLAND_ENGINE=boa`, pure Rust, no prebuilt archive). Engine specifics live in the crate: pinned handle scopes, a parked unwind so a scriptc throw crossing a host function (`process.exit` from island code) re-raises at the outermost entry instead of unwinding through V8 frames, `import.meta.url` for path keys, and dynamic `import()` evaluating in a microtask (V8 forbids evaluating a graph that is mid-evaluation; a cycle through `import()` is exactly that). The V8 crate's build script fetches its prebuilt static library once into `~/.cargo/.rusty_v8` (37 MB; `RUSTY_V8_ARCHIVE` points an offline host at it); a dynamic binary grows by about 45 MB. redcode's native binary, best of three against the same worktree — `--version` 0.9 s (boa 4.4 s, Bun 1.1 s), `--help` 0.9 s (5.3 s, 1.1 s), `stats` 1.4 s (12.2 s, 1.7 s), `session list` 1.4 s (12.2 s, 1.6 s) — with output identical to Bun's. Also: a fired island `Timeout` answers `hasRef()` from Node's ref flag rather than the heap (both engines printed `false` after `ref()`).
- **The red-dev shapes: lazy `import()` of island-tier modules, `uncaughtException`, Dates into the island, switches on handles.** A dynamic `import()` of one of the program's own modules that the frontier classified island now embeds like an npm module and lowers to the engine's `import()` (red-dev's 389 lazy-command sites; corpus 2968). `process.on("uncaughtException", cb)` registers a listener a synchronous throw or the entry's rejection dispatches to, with the loop running on for the listener's async work; an unhandled rejection reaches it when no `unhandledRejection` listener exists (corpus 2970, Rust backend). A native `Date` crossing into island code becomes an engine Date at the same instant (corpus 2969). A `switch` whose scrutinee is an island handle under a static type exits it once into a fresh const, and a clause ending in a block that returns counts as an exit for the union desugar (corpus 2971). Relative `.js` specifiers inside island modules resolve to TypeScript sources (corpus 2967). red-dev's reached diagnostics went 7 → 1 on the old reach; the switch then unlocked the whole program (430 → 1523 statements), which lists its next gaps: records holding a `Set` crossing into the island, `Bun.file().size`, callback signatures.
- **Island memory and engine tuning: shared compiled regexes, a collection policy for long-lived heaps, an ASCII lexer fast path.** The compiled-pattern cache now shares one `Rc<Regex>` per (source, flags) instead of cloning the compiled program per evaluation — emoji-regex compiles to megabytes, and yargs' help allocated 2.8 GB in a second and a half (`redcode --help`: 7.4 s / 2.8 GB → 5.9 s / 483 MB). boa_gc is vendored with a first collection at 64 MB and later thresholds at twice the surviving heap (a CLI keeps its module graph alive; the 1 MB / 70% policy spent about a fifth of every command in full mark-sweeps). boa_parser is vendored with an ASCII fast path in the lexer's identifier tests (ICU code point sets were consulted per character). Both carry a `PATCHES.md`.
- **Island start-up: modules parse on first load, and compiled regexes are cached.** Realm creation parsed every embedded module up front (4,658 sources for redcode — 17 of its 30 boot seconds, reached or not); the loader now parses a module the first time the ES graph or a static import loads it (`redcode --version`: 28 s → 7.7 s, 1.0 GB → 440 MB). Separately, the vendored boa recompiled a regular expression on every evaluation of a literal (regress parsed and optimized the pattern each time): yargs' help layout — string-width's emoji-regex, ansi-regex — spent 56 of 60 profiler samples recompiling, 67 s for one `--help`; a bounded per-thread cache keyed by (source, flags) now clones the compiled matcher (`vendor/boa_engine/PATCHES.md`). `SCRIPTC_KEEP_SYMBOLS=1` keeps the symbol table in a release binary for gdb/perf.
- **`node:child_process` works inside the island.** `spawn`/`exec`/`execFile` deliver Node's `ChildProcess` — an EventEmitter with piped stdin/stdout/stderr streams (or ignored/inherited slots), `pid`, `exitCode`/`signalCode`, `kill`, `unref`, and the `spawn`/`exit`/`close`/`error` events in Node's order (a missing binary is an `error` event carrying ENOENT, errno, syscall, path) — over the same async child unit the static lane lowers to, through the loop-side reentry seam the socket bridge uses. The kernel gains a piped stdin with write/end. `spawnSync`/`execSync`/`execFileSync` run over the synchronous spawn core, which now takes a working directory; `execSync` inherits stderr by default like Node. Effect's command executor and redcode's shell utilities drive exactly this surface; `redcode models` stopped at `child_process.spawn`. The ambient surface declares the members the island serves (the static tier keeps fencing them per member). Corpus 2966.
- **`bun:sqlite` runs in the island over a native SQLite kernel.** The Rust runtime gains a `sqlite` feature (rusqlite 0.40.1 with the bundled amalgamation — the C compiles inside cargo like `ring`'s; the program TU stays `forbid(unsafe_code)`), switched on only for graphs whose embedded code imports `bun:sqlite` (the IR now lists the Bun runtime modules a build reaches). The island's `bun:sqlite` facade replaces the trap when the host exposes the bridge: `Database` (query/prepare/run/exec/transaction with savepoint nesting/serialize/close), `Statement` (all/get/values/run/iterate/safeIntegers/as/columnNames/paramsCount/finalize), positional and `$name`/`:name`/`@name` bindings, blobs as `Uint8Array`, `safeIntegers` answering bigint, and `SQLiteError` with Bun's `code` (`SQLITE_CONSTRAINT_UNIQUE`) and `errno`. Values cross as engine values, not JSON. The ambient surface declares the module for projects without `@types/bun`. Corpus 2965 (Bun oracle, byte for byte). `redcode models` now gets past `new Database(...)`.
- **The island's `node:fs` works through descriptors.** `openSync`/`closeSync`/`readSync`/`writeSync`/`fstatSync`/`ftruncateSync`/`fsyncSync`, their callback spellings in every Node overload (`read(fd, {buffer, position}, cb)` included), and `fs.promises.open` returning a `FileHandle` (read/write/readFile/writeFile/stat/truncate/sync/close) now run over the runtime's open-file table instead of throwing "whole-file reads/writes only" — Effect's FileSystem opens a descriptor and reads/writes at positions, which is how `redcode models` reached the refusal. Numeric flags map to Node's flag strings; positions may be bigint. Island fs errors carry `syscall`, `path`, `dest`, and `errno` beside `code`, read off the runtime's Node-shaped message. The ambient `node:fs` surface declares the same members (the static tier fences them per member as before). Corpus 2964.
- **A CommonJS facade never reads a non-enumerable accessor export.** Under `--target bun` the facade lists every syntactic export, and binding `Object.defineProperty(exports, "Promise", { get() { console.warn(…) } })` ran the getter at import — ioredis printed its "third-party Promise" warning from every redcode command. Bun enumerates own keys, so a non-enumerable accessor is left unread (its binding stays undefined); an enumerable getter is still read once, at facade evaluation. Corpus 2960.
- **Async static callbacks cross into the island as engine promises.** A scriptc `async` closure handed to an embedded module (yargs middleware, an Effect combinator) used to refuse with "outside the JSON-safe island subset"; its native promise now becomes a pending engine promise the island can `await`, settled from the native continuation — a fulfilled dyn or JSON-safe value marshals in, a rejection arrives as the engine's own `Error` class with the reason's message and `code`. The plain `island_call` funnel drains the engine's job queue after the call (the method funnel already did), so a settlement from a timer or I/O continuation wakes the awaiting island code instead of leaving its top-level await pending. A typed callback's `number`/`boolean`/`string` parameter arriving as a handle through the dyn bridge now exits strictly (the checked-dynamic scalar checks gained an island arm) instead of failing with "expected number, got object". A callback that crossed as a plain dyn function (yargs' `.middleware(cb)`, an `any`-typed parameter) may answer a promise too. The C/LLVM lanes refuse an async callback answering a composite with an internal error naming `--backend rust`. Corpus 2963. With this, the redcode binary prints its full yargs `--help` (identical to Bun's) and `--version`.
- **The Rust island runs redcode's whole dependency graph at boot.** Everything the ~3,750 embedded modules reach for at module evaluation now exists: Intl with bundled ICU (plus a currency/percent NumberFormat layer boa lacks), the web globals (Blob/File/FormData, EventTarget and the Event classes, MessageChannel/MessagePort, BroadcastChannel, structuredClone, performance, navigator, WritableStream, TextEncoderStream), the Node 24 member surface of every shimmed builtin (util.aborted, worker_threads.markAsUncloneable, stream.destroy, fs descriptor fences…), eleven more builtins (http2, dgram, cluster, inspector, vm, sea, trace_events, repl, wasi, test, sqlite) that link with Node's shape and fence at the call, and Node 24's full export names on every builtin facade. Engine limits rise to a CLI's depth (8192 frames, a 4M-slot value stack; the program runs on a 256 MB-stack thread), `SCRIPTC_ISLAND_TRACE=1` renders boa's backtrace into island errors, and `cargo run --example island_parse|island_eval|island_module` probe the engine ahead of a build.
- **Bun semantics for embedded packages under `--target bun`.** Package resolution follows Bun's runtime ("main" answers when there is no exports map; the "module" field is a bundler convention), a CommonJS module's ESM facade exposes every key Bun's evaluation would (a permissive syntactic superset over Node's lexer names), ES modules that call `require` at top level get a module-bound one, ".js" files that spell ES syntax inside CommonJS-typed packages embed as ESM, and text (`.txt`/`.md`) and file assets imported by embedded modules become string and path modules.
- **Fixes in the engines.** boa 0.22.0 is vendored with one guard: its property inline cache paired the shape read after an accessor call with the slot recorded before it, so a getter that redefines its own property (zod v4's lazy `shape`, the shape every MCP schema spreads) made every later read at that site call the stored value. boa also refused `export { … }` lists ending a file without a semicolon (such sources take the transpile path) and overflowed an 8 MB stack on generated validators. The C island sized its builtin facade wrapper in a 2 KB stack buffer, which Node 24's export lists overflow.
- **Island modules import text and file assets under `--target bun`.** An embedded module's `import prompt from "./initialize.txt"` (or any extension `with { type: "text" }`) embeds as a string module carrying the file's content, other extensions as a path — Bun's loaders, the static tier's asset rule; Node targets keep `ERR_UNKNOWN_FILE_EXTENSION` at evaluation. Previously the raw text reached the engine as JavaScript. An island parse failure now names the embedded module and quotes how its source begins. Corpus 2957 (bun oracle).
- **`SCRIPTC_RUST_REFUSALS=all` names every Rust backend gap in one build.** A refusal drops the function's partial output and emission continues, so a large program reports all of its SC3001s at once instead of the first; `if (await gate())` over a union-typed promise (a truthiness or unary wrapper around a suspension) now lowers on the Rust lane.
- **The Rust backend suspends inside `if` under try/catch/finally.** `try { if (help) { await cli.parse(...) } else { await cli.parse() } } catch ... finally ...` — the CLI entry shape — lowers on the Rust lane: each branch continues with the rest of the protected segment in a helper, the condition evaluating inside the protected segment so a throw there stays caught. Previously a named SC3001 refusal. Corpus 2951 (both lanes).
- **Typed callbacks into the island accept `Error`, `Error | undefined`, and `unknown` parameters.** The Node-style `(err, result, out)` callback shape (`cli.parse(args, (err: Error | undefined, argv: unknown, out: string) => ...)` against yargs) crosses as a host function: the realm's Error copies out as a native error (name and message), an absent or undefined argument takes the unit arm, and an `unknown` parameter keeps the engine value as a handle inside the checked-dynamic value. Records, arrays, and undefined-armed JSON unions convert through the same exit pipeline on the Rust lane. Corpus 2956.
- **Qualified reads through a namespace of island handles resolve to the member.** `Ns.member` and `Ns.f(...)` where the namespace maps to a handle type (a static module whose exports are island-typed values, a barrel over an island module, a namespace re-exported under a name) resolve the member like a bare identifier instead of materializing the namespace object. Corpus 2955.
- **A `const` aliasing a module namespace is plumbing, not a value.** `export const Event = ServerEvent` (the re-export-under-a-new-name style) registers no storage and lowers to no statement: `Event.Connected` resolves through the namespace exactly like `ServerEvent.Connected` (live bindings, island handles included), chains of aliases resolve link by link, and a bare `Event` used as a value materializes the namespace record at the use site. Corpus 2954.
- **`export *` barrels pass island exports through; namespaces that re-export themselves lower.** A static barrel whose `export * from "./inner.ts"` names an island module binds every value export of that module as a handle, keyed by the same symbols a consumer's alias chain lands on, so `import { x } from "./barrel.ts"` works exactly as a direct island import. A module that re-exports its own namespace (`export * as Ns from "."` inside `.`) now materializes as a record whose self member is omitted — the object cannot hold itself — in both the mapped shape and the literal (a documented divergence beside key order and identity; reading the self member is a named fence). Island modules also resolve extensionless directory imports to `index.tsx`.
- **Island modules keep their tsconfig `paths` aliases.** A program module classified island (`--island-module`, `scriptc.json` pins, or the automatic frontier) that imports `@/helper` through the project's `paths` table embeds the aliased program file by realpath, exactly as its static importers resolve it; package code under `node_modules` never consults the table. On the redcode dogfood this was the single root cause behind 235 of the 244 remaining diagnostics.
- **Callbacks from static code re-enter the island and carry their arity.** A scriptc closure handed to an embedded module (solid's `createRoot`, an Effect combinator) may call back into the realm while it runs — reading a signal, calling a handle it received — without tripping the island's single borrow: the boundary swaps in a spare engine context for the duration and restores it on the way out (safe Rust, no `unsafe`). The host function the realm sees reports the closure's declared `length`, so libraries that branch on a callback's arity (`createRoot` only hands a disposer to callbacks declaring one) take the same path as on Node. `typeof` on an island handle asks the engine instead of answering `"object"`, and a callback answering a handle (a nested accessor, a disposer) hands it back as a handle. Array destructuring of an island value (`const [get, set] = createSignal(0)`) iterates the realm's array.
- **The Rust island gains `URL`, `DisposableStack`, and safe `using`.** The engine (boa) had no `URL` global, no `DisposableStack`/`AsyncDisposableStack`/`SuppressedError`, and compiled `using` declarations without disposing them (a `using` inside a function aborted the engine). The island now defines a WHATWG `URL` over the runtime's own parser (components, base-relative parsing, setters, a `searchParams` bound to the URL, `canParse`/`parse`, Node's `ERR_INVALID_URL`), the shared prelude polyfills the disposable-stack classes where the engine lacks them, and embedded sources — workspace TypeScript and any JavaScript carrying `using`/`await using` — downlevel to ES2022 so disposal runs through try/finally.
- **Awaited island promises exit to their declared type.** `await` of an engine promise typed `Promise<T>` with a boundary-validatable `T` (a primitive, a JSON-safe record or array) now yields `T`, the same validated exit island call results take, so `console.log(await islandFn())` no longer fences on a handle.
- **Module namespace objects are values.** `import * as Ns` / `export * as Ns` used as a value — passed to a function, enumerated, stored in a record, read through a structural type — lowers to a record of the module's value exports (constants, declared functions, nested namespaces) built at the use site. Two divergences are documented: key order follows the record's declaration order where Node's namespace is alphabetical, and identity is per use (`Ns === Ns` sees fresh records). An export the record cannot hold (a mutable `export let`, a class, an optional/rest-parameter function) keeps the SC1013 fence, now naming the member.
- **Bun runtime modules trap inside the island under `--target bun`.** Embedded npm code importing `bun:sqlite`, `bun:ffi`, or `bun` links against a trap table: the import succeeds, `typeof` answers `function`, property reads keep answering traps, and only a call or construction throws naming the member and the runtime — the island twin of the static tier's trap-on-call. The coverage report lists each trapped module; a Node target still refuses to embed the package. Island-side `import * as ns` bindings answer the module namespace, bare `.`/`..` specifiers resolve as relative directories, and a primitive-typed island export reads as its declared primitive instead of a handle.
- **`--island-module <glob>` starts the static frontier.** A program module named by the flag leaves the static tier: it embeds as engine source exactly like an npm module, static code binds its exports as engine handles (calls, namespace reads, and primitive results validated on exit), and the coverage report gains a "static frontier" section counting native modules against island ones with each island module's reason. Requires `--dynamic`; the corpus directive `// @island-module: <spec>` pins programs mixing both tiers.
...
- **Synchronous child commands now run through the safe-Rust backend.** `execFileSync`, shell-backed `execSync`, and result-returning `spawnSync` support UTF-8 capture, stdin input, cwd and environment selection, pipe/ignore/inherit stdio, timeouts, exit status and signals, and spawn errors with Node-compatible messages and codes.
- **The safe-Rust runtime now has its first event-loop phases.** Timeout and Immediate handles, repeating intervals, clear/ref/unref/hasRef/refresh operations, active-resource snapshots, `process.nextTick`, and `queueMicrotask` retain typed closures, preserve synchronous/next-tick/microtask/timer/check ordering and liveness, accept work queued from callbacks, and drain before executable shutdown without busy-waiting.
- **Rust programs can use wall/monotonic clocks and synchronous waits.** `Date.now()`, `process.uptime()`, `performance.now()`, and the finite-timeout `Atomics.wait` sleep idiom preserve Node's ordering and timing invariants without polling or unsafe code.
- **Process introspection is available to safe-Rust programs.** CPU and thread-CPU samples and deltas, resource-usage records, cgroup-aware memory readings, and active event-loop resources preserve Node's public record shapes and validation behavior.
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve` and `Promise.reject`, `new Promise(resolve => ...)`, typed executor `reject` callbacks, and pending executors that ignore `resolve`, same-type `Promise.all`, union-adapting `Promise.race`, fulfillment-only `.then()`, awaited console arguments, string conversions and concatenations including inside protected segments, binary expressions, record fields, indexed array reads, typed-byte construction, array-intrinsic arguments, and `Map.set` values whose receivers and earlier arguments stay live across suspension, rejection-aware `try/catch/finally` segments with awaited cleanup, return-through-finally, and completion replacement, suspended lexical blocks and `if` branches plus `for`, `for-of`, `while`, and protected `for-of` loops, statement- and value-position non-Promise await hops, captured bindings, and timer- and Immediate-backed promises resume through the runtime microtask queue, propagate typed results and rejections, and preserve first-settlement-wins behavior. Branch and loop continuations retain their live locals; loops preserve initializer/condition/update state and protected-body completion. Unhandled rejections are reported only after the microtask checkpoint and exit nonzero after final safe-Rust heap cleanup; more complex suspension shapes still refuse explicitly.
- **Rust exception snapshots preserve primitive payloads.** `typeof` narrowing and `String(e)` over caught numbers, booleans, strings, and Error objects now work in synchronous and async rejection paths instead of limiting catch bindings to Error-only operations.
- **User Error subclasses survive polymorphic Rust exception paths.** A traced per-module representation carries native Errors and concrete subclasses through built-in `Error`-typed parameters and slots without erasing their safe-heap identity. `super(message)` initializes inherited state; added fields, methods, exact-subclass and built-in-ancestor `instanceof` narrowing, `Error.prototype.toString()`, `String(e)` and template interpolation on catch bindings, synchronous catches/rethrows, async rejection chains, and Error/closure cycles preserve Node behavior and finish with a clean heap audit. Uncaught synchronous throws are reported only after that cleanup and exit with Node's status 1 instead of Rust's panic status 101.
- **Rust awaits preserve optional Promise values.** `Promise<T> | undefined` and `Promise<void> | undefined` take a real microtask hop on unit arms, await and retag fulfilled Promise arms, and forward rejections through the generated state machine.
- **`Promise.withResolvers()` is available in Rust output.** String, number, and void records expose traced `promise`, `resolve`, and typed-Error `reject` closures that settle across timer callbacks and preserve first-settlement-wins behavior.
- **The Rust backend now handles synchronous filesystem workflows.** UTF-8 and byte-exact whole-file and descriptor I/O, positioned reads and writes, appends, existence checks, directory listing and mutation, recursive removal, temporary directories, permissions, copying, renaming, canonical paths, and `statSync`/`lstatSync` metadata match Node across success, normalized errno messages, `try/catch/finally`, and typed JSON configuration round-trips.
- **`fs/promises` workflows are available in Rust output.** UTF-8 and byte reads, UTF-8 writes with ordered encoding/mode options and Node-compatible mode validation, directory creation and listing, removal, unlinking, chmod, renaming, and stat settle typed promises, convert filesystem throws into catchable rejections, and compose with `Promise.all` while preserving input order.
- **Callback-style `fs.rename` uses a native Rust worker pool.** Four bounded workers make filesystem progress while JavaScript is occupied; callbacks and GC values remain on the main thread, requests keep the event loop alive, and one completion is delivered per `nextTick`/microtask checkpoint with typed `Error | null` results.
- **Safe-Rust FileHandles have shared descriptor ownership and complete data operations.** `fs.promises.open`, current-offset and positioned `read`/`write`, whole-file string and Buffer I/O, `fd`, `stat`, and idempotent `close` preserve Buffer identity, alias-visible close state, Node-compatible rejections, and close a still-open descriptor when the final handle is released.
- **Typed arrays and Buffer have a safe-Rust storage model.** Numeric element coercion, copying, shared views, slicing, bulk set, snapshot sorting, non-mutating `toReversed`/`with`, array conversion, numeric `join`, UTF-8/hex/base64 conversion, concatenation, fixed-width numeric access, iteration, and byte-exact synchronous filesystem I/O are differential-tested against Node.
- **Rust containers now cover array identity searches, ranges, removals, mutations, and ES2023 copying plus insertion-ordered `Map` and `Set`.** Strict-equality `indexOf`, SameValueZero `includes`, shallow `slice`, removal-only `splice`, optional `shift`, variadic and snapshot-spread `unshift`, identity-preserving `reverse`, non-mutating `toReversed`/`toSpliced`/`with`, cross-signature function-union identity, number/string keys, seeded construction, live mutation during iteration, reference values, cycle tracing, and first-class constructor registries preserve Node behavior in safe Rust.
...
- **Rust optional chaining now narrows static unit-armed unions.** Receivers evaluate once, null/undefined paths stay lazy and produce the result union's undefined arm, and present reference payloads retain identity through chained reads.
