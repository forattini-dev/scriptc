...
- **Rust containers now cover array identity searches and mutations plus insertion-ordered `Map` and `Set`.** Strict-equality `indexOf`, SameValueZero `includes`, variadic and snapshot-spread `unshift`, identity-preserving `reverse`, number/string keys, seeded construction, live mutation during iteration, reference values, cycle tracing, and first-class constructor registries preserve Node behavior in safe Rust.
