...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve`, `new Promise(resolve => ...)`, same-type `Promise.all` and `Promise.race`, fulfillment-only `.then()`, direct awaited console arguments, non-Promise await hops, captured bindings, and timer- and Immediate-backed promises in straight-line async functions resume through the runtime microtask queue, propagate typed results and rejections, and retain safe-Rust heap auditing; more complex suspension shapes still refuse explicitly.
