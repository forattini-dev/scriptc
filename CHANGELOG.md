...
- **Concrete user Error subclasses survive Rust exception paths.** `super(message)` initializes inherited `name`/`message` state, added fields remain available after synchronous throws and async rejection/rethrow chains, exact subclass and built-in-ancestor `instanceof` checks narrow caught values, and direct `Error.prototype.toString()` calls preserve the name/message format; polymorphic storage typed as a built-in Error remains outside this slice.
