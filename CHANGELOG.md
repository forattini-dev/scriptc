...
- **Safe-Rust FileHandles have shared descriptor ownership.** `fs.promises.open`, `fd`, `stat`, and idempotent `close` preserve alias-visible close state, return the same metadata shape as path stat, and close a still-open descriptor when the final handle is released.
