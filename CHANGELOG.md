...
- **The safe-Rust runtime now has its first event-loop phases.** One-shot `setTimeout`, `setImmediate`/`clearImmediate`, and `queueMicrotask` retain typed closures, preserve synchronous/microtask/timer/check ordering, accept work queued from callbacks, and drain before executable shutdown without busy-waiting.
