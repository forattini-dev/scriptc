...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve` values and straight-line awaits in non-capturing async functions resume through the runtime microtask queue, propagate typed results and rejections, and retain safe-Rust heap auditing; more complex suspension shapes still refuse explicitly.
