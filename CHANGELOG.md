...
- **Basic `fs/promises` whole-file I/O is available in Rust output.** UTF-8 and byte reads plus UTF-8 writes settle typed promises, convert filesystem throws into catchable rejections, and compose with `Promise.all` while preserving input order.
