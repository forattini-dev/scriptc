...
- **Qualified reads through a namespace of island handles resolve to the member.** `Ns.member` and `Ns.f(...)` where the namespace maps to a handle type (a static module whose exports are island-typed values, a barrel over an island module, a namespace re-exported under a name) resolve the member like a bare identifier instead of materializing the namespace object. Corpus 2862.
- **A `const` aliasing a module namespace is plumbing, not a value.** `export const Event = ServerEvent` (the re-export-under-a-new-name style) registers no storage and lowers to no statement: `Event.Connected` resolves through the namespace exactly like `ServerEvent.Connected` (live bindings, island handles included), chains of aliases resolve link by link, and a bare `Event` used as a value materializes the namespace record at the use site. Corpus 2861.
