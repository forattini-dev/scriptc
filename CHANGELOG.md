...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve`, `new Promise(resolve => ...)`, fulfillment-only `.then()`, non-Promise await hops, and timer- and Immediate-backed promises in straight-line async functions resume through the runtime microtask queue, propagate typed results and rejections, and retain safe-Rust heap auditing; capturing async closures and more complex suspension shapes still refuse explicitly.
