...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve` and `new Promise(resolve => ...)` values, non-Promise await hops, and timer- and Immediate-backed promises in straight-line, non-capturing async functions resume through the runtime microtask queue, propagate typed results and rejections, and retain safe-Rust heap auditing; more complex suspension shapes still refuse explicitly.
