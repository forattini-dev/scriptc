...
- **Rust containers now cover array identity searches, ranges, removals, and mutations plus insertion-ordered `Map` and `Set`.** Strict-equality `indexOf`, SameValueZero `includes`, shallow `slice`, removal-only `splice`, optional `shift`, variadic and snapshot-spread `unshift`, identity-preserving `reverse`, cross-signature function-union identity, number/string keys, seeded construction, live mutation during iteration, reference values, cycle tracing, and first-class constructor registries preserve Node behavior in safe Rust.
