...
- **The safe-Rust runtime now has its first event-loop phases.** `setTimeout` handles, repeating intervals, clear/ref/unref/refresh operations, `setImmediate`/`clearImmediate`, `process.nextTick`, and `queueMicrotask` retain typed closures, preserve synchronous/next-tick/microtask/timer/check ordering and liveness, accept work queued from callbacks, and drain before executable shutdown without busy-waiting.
