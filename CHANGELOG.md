...
- **The Rust backend now handles synchronous text filesystem workflows.** UTF-8 reads, writes, appends, existence checks, directory listing and mutation, file removal, and canonical paths match Node across success, error propagation, `try/catch/finally`, and typed JSON configuration round-trips.
