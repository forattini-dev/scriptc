...
- **The Rust backend has its first generated async state machines.** Typed `Promise.resolve` and observed `Promise.reject`, `new Promise(resolve => ...)`, same-type `Promise.all` and `Promise.race`, fulfillment-only `.then()`, awaited console arguments, binary expressions, and record fields, rejection-aware `try/catch` segments, non-Promise await hops, captured bindings, and timer- and Immediate-backed promises in straight-line async functions resume through the runtime microtask queue, propagate typed results and rejections, and retain safe-Rust heap auditing; more complex suspension shapes still refuse explicitly.
