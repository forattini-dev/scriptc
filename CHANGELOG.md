...
- **`fs/promises` workflows are available in Rust output.** UTF-8 and byte reads, UTF-8 writes, directory creation and listing, removal, unlinking, chmod, renaming, and stat settle typed promises, convert filesystem throws into catchable rejections, and compose with `Promise.all` while preserving input order.
