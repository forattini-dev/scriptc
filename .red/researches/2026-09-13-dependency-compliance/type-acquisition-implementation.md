...
A ampliação da admissão AUTO de JS com `@types` e a preservação conservadora de tipos estruturais foram implementadas no [ciclo seguinte](./auto-third-party-types.md). A aquisição sozinha não resolve os bloqueios dos corpos: o experimento real com `is-extglob` agora chega à recusa de regex global. Continuam pendentes os bloqueios de inferência/Promise, famílias de APIs nativas e substituição das integrações estrangeiras. `ring` e SQLite existentes ainda impedem afirmar pureza Rust de todo o runtime.
