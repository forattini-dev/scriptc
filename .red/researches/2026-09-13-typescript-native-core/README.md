...
### Sexto checkpoint: chaves computadas no frontend

O corpus 1703 agora compila e executa com paridade Node em Rust sem engine, sem runtime fences. A conversão das duas representações de objeto com chaves string foi centralizada, admitindo a representação jsval por uma conversão já existente. O grupo completo de protótipos dinâmicos passou; quatro testes focados passaram em plain e na configuração sanitized. O armazenamento dinâmico por símbolo continua fora desse contrato. Ver [implementação, limites e evidências](computed-string-keys.md).
...
### Oitavo checkpoint: valores de função com rest tipado

O IR e o frontend agora preservam o array final de uma função com rest quando ela vira callback, fica armazenada em campos ou é retornada por outra função. O backend Rust executa essas chamadas pelo ABI tipado. Os diferenciais novos passaram sem engine e sem runtime fences; a suíte inteira de diagnósticos passou com 121/121. As conversões para callbacks dinâmicos ainda não implementadas continuam recusadas.

A nova análise do Redcode original terminou em 273,77 s e pico de 4,839 GiB, sem erros internos, sob o mesmo perfil experimental de uma CPU e GOMEMLIMIT=4GiB. Os mesmos 1.335 textos-fonte conservaram ordem e hashes. Os diagnósticos passaram de 3.087 para 3.078; as mensagens que apontavam rest passaram de 16 para duas. A análise agora chega a bloqueios posteriores de filtros, optional chaining e uniões. Ainda não há binário Redcode, e os gates gerais continuam abertos. Ver [implementação, evidências, limpeza de cache e limites](typed-rest-function-values.md).

### Nono checkpoint: filtros comprovados e efeitos nas comparações

O frontend agora comprova predicates escritos que excluem undefined, preservando a distinção de comparações estritas/não estritas, a identidade do parâmetro e a proteção contra nomes locais. As comparações ordinárias com undefined reutilizam essa prova. Também foi corrigida uma simplificação que eliminava chamadas, exceções e incrementos quando o resultado de uma comparação nullish era constante.

A seleção focada passou com 16 testes; a seleção sanitized com 10, incluindo os novos corpus C e os contratos Rust sem engine; os diagnósticos completos passaram com 121/121, sem mudanças de snapshot. O Redcode original terminou a análise com 3.072 diagnósticos, contra 3.078, mantendo os mesmos 1.335 textos-fonte e hashes. Tempo de 465,93 s e pico de 4,920 GiB no perfil experimental; não há comprovação de ganho de desempenho nem binário Redcode. Os gates gerais continuam bloqueados pela falha C preexistente de library-contract. Ver [provas, efeitos, resultados e limites](nullish-filter-proof.md).
