...
async function build() {
  ...
  const parallel = async (items, task) => {
    const requestedWidth = Number(process.env.SCRIPTC_NATIVE_WORKERS ?? "2");
    if (!Number.isInteger(requestedWidth) || requestedWidth < 1) throw new Error("SCRIPTC_NATIVE_WORKERS must be a positive integer");
    const width = Math.max(1, Math.min(requestedWidth, availableParallelism()));
