...
Requires clang on the PATH (Xcode Command Line Tools on macOS, `clang` package on Linux). Experimental `--backend rust` builds require Cargo and rustc instead of clang.

The Rust backend is an explicit native-only subset. It emits
