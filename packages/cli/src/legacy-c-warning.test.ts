...
test("warns only for generated legacy C executables on runtime-pack hosts", () => {
  expect(shouldWarnLegacyCExecutable({ ...executable, backend: undefined }, { SCRIPTC_CC: "clang" }, true)).toBe(false);
  expect(shouldWarnLegacyCExecutable({ ...executable, backend: "rust" }, { SCRIPTC_CC: "clang" }, true)).toBe(false);
