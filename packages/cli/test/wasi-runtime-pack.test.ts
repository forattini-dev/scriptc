...
import { nativeHelperForTarget, WASM32_WASI_TARGET } from "../../compiler/src/backend/targets.js";
...
const helperSpec = nativeHelperForTarget(WASM32_WASI_TARGET);
const helperPackage = helperSpec === null ? null : require.resolve(`${helperSpec.packageName}/package.json`);
const helper = helperPackage === null ? null : join(
  dirname(helperPackage), "bin", process.platform === "win32" ? "scriptc-llvm-codegen.exe" : "scriptc-llvm-codegen",
);
...
const supported = helper !== null && existsSync(helper) && existsSync(runtimeManifest);
const dirs: string[] = [];
...
test.runIf(supported)("WASI helper object plus runtime pack builds and runs without SCRIPTC_CC", async () => {
  ...
  await expect(execFileAsync(process.execPath, [
    "--import", tsxLoader, cliEntry, "run", join(repoRoot, "tests/corpus/001-hello.ts"), "--backend", "llvm", "-o", output,
  ], {
    env: {
      ...process.env,
