import { nativeCodegenTarget } from "../../compiler/src/backend/targets.js";
...
const supported = target !== null && process.platform !== "win32";
const dirs: string[] = [];
...
describe.runIf(supported)("POSIX native outputs", () => {
  test("default names and exact paths identify assembly and object artifacts", async () => {
    ...
    const magic = target!.objectFormat === "elf" ? [0x7f, 0x45, 0x4c, 0x46] : [0xcf, 0xfa, 0xed, 0xfe];
    expect((await readFile(exact)).subarray(0, 4)).toEqual(Buffer.from(magic));
  });
