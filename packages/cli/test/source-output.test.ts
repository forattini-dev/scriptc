...
test("default and explicit paths identify each source primary artifact", async () => {
  ...
  const expected = { ir: "hello.ir.json", c: "hello.c", rust: "hello.rs", llvm: "hello.ll" } as const;
  for (const kind of ["ir", "c", "rust", "llvm"] as const) {
...
test("an explicit default-shaped output path preserves caller-owned siblings", async () => {
  const { dir, entry } = await fixture();
  const siblings = ["hello", "hello.exe", "hello.wasm", "hello.c", "hello.rs", "hello.ll", "hello.s", "hello.o"];
...
test("source outputs never execute compiler, archiver, or linker traps", async () => {
  ...
  await mkdir(traps);
  for (const tool of ["cargo", "rustc", "clang", "cc", "gcc", "zig", "ar", "ld", "link", "link.exe", "xcrun"]) {
