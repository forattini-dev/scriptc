...
import { parseShardSpec } from "../../../tests/harness/shard.js";
...
/* Corpus parity for the Rust backend is auto-discovered by
 * tests/harness/rust-differential.test.ts (every corpus program, no
 * hand-lists). The generated-source shape assertions the old corpus
 * blocks carried live on here. */
test("Rust generated-source shapes hold for representative corpus programs", async () => {
  const cases: ReadonlyArray<{ fixture: string; assert: (source: string) => void }> = [
    {
      fixture: "700-classes-basic.ts",
      assert: (source) => {
        expect(source).toContain("struct sc_o_Point");
        expect(source).toContain("runtime::Gc<sc_o_Point>");
        expect(source).not.toContain("extern \"C\"");
      },
    },
    {
      fixture: "2483-recursive-record-cycles.ts",
      assert: (source) => expect(source).toContain("enum sc_u_u0"),
    },
    {
      fixture: "420-dead-strip-modules/main.ts",
      assert: (source) => {
    ...
    {
      fixture: "2470-mockable-module-shape.js",
      assert: (source) =>
        expect(source).toContain("Cannot add property '{}' to a fixed-shape object"),
    },
    {
      fixture: "2538-destructuring-assign-class-source.ts",
      assert: (source) =>
        expect(source).toMatch(/sc_fld_miss: Some\(sc_u_[A-Za-z0-9_]+::ScArm\d+\)/u),
    },
    {
      fixture: "2486-recursive-record-boundaries.ts",
      assert: (source) => {
  ...
  for (const { fixture, assert } of cases) {
    const entryPath = resolve("tests/corpus", fixture);
    const dir = await mkdtemp(join(tmpdir(), "scriptc-rust-shape-"));
    const dynamic = (await readFile(entryPath, "utf8")).split("\n", 4)
      .some((line) => /^\/\/ @dynamic\s*$/u.test(line));
    const result = await compile(entryPath, {
      outDir: dir,
      outPath: join(dir, "program-shape"),
      backend: "rust",
      dynamic,
      optimization: "dev",
    });
    ...
    assert(await readFile(result.sourcePath, "utf8"));
  }
}, 240_000);

/** Runs an executable with its stdout pipe destroyed immediately, so the
 * program's writes hit EPIPE mid-stream. Returns exit code and stderr. */
function runWithClosedStdout(
  file: string,
  args: string[],
  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.on("close", (code, signal) => {
      if (signal !== null) rejectRun(new Error(`killed by ${signal}: ${stderr}`));
      else resolveRun({ exitCode: code ?? 0, stderr });
    });
  });
}

test("Rust stdout EPIPE is swallowed like Node: the program survives a closed pipe", async () => {
  // Node's own stance (and the C runtime's): process.stdout/console writes
  // never throw or abort on EPIPE -- `program | head` finishes cleanly.
  // The stderr marker proves execution continued past the dead pipe.
  const dir = await mkdtemp(join(tmpdir(), "scriptc-rust-epipe-"));
  const entryPath = join(dir, "epipe.ts");
  await writeFile(entryPath, `
...
`);
  ...
  if (!result.ok) return;
  const env = { ...process.env, SCRIPTC_RUST_HEAP_AUDIT: "1" };
  const [node, rust] = await Promise.all([
    runWithClosedStdout(nodeOracleExecutable(), [entryPath], env),
    runWithClosedStdout(result.binaryPath, [], env),
  ]);
  expect(node).toEqual({ exitCode: 0, stderr: "survived\n" });
  expect(rust, "rust binary must survive stdout EPIPE the way Node does").toEqual(node);
}, 240_000);
