...
test("Rust process output completion callbacks match Node", async () => {
  const fixture = resolve("tests/corpus/2687-process-write-args.ts");
  const dir = await mkdtemp(join(tmpdir(), "scriptc-rust-process-write-"));
