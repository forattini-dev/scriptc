...
test("static backends honor String.prototype.lastIndexOf position", async () => {
  const fixture = resolve("tests/corpus/2825-string-last-index-of-position.ts");
  const node = await execFileAsync(nodeOracleExecutable(), [fixture]);
  expect(node.stdout).toBe("2\n");

  for (const backend of ["c", "llvm", "rust"] as const) {
    const dir = await mkdtemp(join(tmpdir(), `scriptc-${backend}-string-last-index-of-`));
