...
test("Rust source is an exact primary artifact without a native build", async () => {
  const { entry, outDir } = await fixture();
  const outPath = join(outDir, "program.rs");
  const result = await compile(entry, { outDir, outPath, outputKind: "rust" });
  expect(result).toMatchObject({ ok: true, artifact: { kind: "rust", path: outPath } });
  expect(await readFile(outPath, "utf8")).toContain("fn main()");
  expect(await readdir(outDir)).toEqual(["program.rs"]);
});

test.each([
  ["rust", "llvm"], ["c", "rust"], ["llvm", "rust"], ["obj", "rust"], ["asm", "c"], ["ir", "rust"],
] as const)("API rejects output %s with backend %s before writing artifacts", async (outputKind, backend) => {
  const { entry, outDir } = await fixture();
  const result = await compile(entry, { outDir, outPath: join(outDir, "artifact"), outputKind, backend });
  expect(result).toMatchObject({ ok: false, diagnostics: [{ code: "SC3002" }] });
  await expect(readdir(outDir)).rejects.toMatchObject({ code: "ENOENT" });
});
