...
test("static backends honor String.prototype.endsWith endPosition", async () => {
  const fixture = resolve("tests/corpus/2827-string-ends-with-position.ts");
  ...
  for (const backend of ["c", "llvm", "rust"] as const) {
    const dir = await mkdtemp(join(tmpdir(), `scriptc-${backend}-string-ends-with-`));
