...
export function emitRustHttpCall(
  ...
  if (expr.fn === "fetch.abortTimeout" && expr.args.length === 1 &&
      expr.args[0]?.type.kind === "dyn" && expr.type.kind === "dyn") {
    const delay = context.nextTemporary();
  ...
  if (expr.fn === "fetch.abortNow" && expr.args.length === 1 &&
    ...
    const dyn = context.dynTypeName();
    return `{ let ${reason} = ${context.emitExpr(expr.args[0])}; let ${reason} = if matches!(&${reason}, ${dyn}::Undefined) { sc_dyn_abort_default_reason() } else { ${reason} }; ${dyn}::AbortSignal(runtime::abort_signal_new_aborted(${reason})) }`;
  }
  ...
  if (expr.fn === "fetch.start" && expr.args.length === 2 &&
    ...
    const url = context.nextTemporary();
    const init = context.nextTemporary();
  ...
  if (expr.fn === "fetch.streamNew" && expr.args.length === 1 && expr.args[0]?.type.kind === "dyn") {
    return `sc_web_stream_new(${context.emitExpr(expr.args[0])})`;
  }
    ...
    const fields = shape.fields.map((field) => {
      ...
      if (field.name === "value" && field.type.kind === "union") {
        ...
        if (union.arms.length === 2 && bytes >= 0 && empty >= 0) {
          ...
          // Reading a stream transfers the chunk reference; a checked-dynamic
          // value copy here would lose identity and copy every HTTP body chunk.
          const checked = `match sc_value { ${dyn}::Bytes(bytes) | ${dyn}::Buffer(bytes) => runtime::live_dyn_ref_get(bytes.identity()).unwrap_or(bytes), value => sc_dyn_check_fail("bytes", &value), }`;
          const value = `match sc_chunk { Some(sc_value) => ${name}::${context.unionVariant(bytes)}(${checked}), None => ${name}::${context.unionVariant(empty)}, }`;
          return `${mangleField(field.name)}: Some(${value})`;
        }
      }
      if (field.name === "value") {
        const value = context.emitDynCheckValue(field.type, `sc_chunk.unwrap_or(${dyn}::Undefined)`, expr.loc);
