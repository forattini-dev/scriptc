...
import { RustStreamPipeEmitter } from "./stream-pipe.js";
...
export class RustEventEmitterEmitter {
  ...
  private readonly streamPipes: RustStreamPipeEmitter;
  ...
  constructor(private readonly context: RustEventEmitterContext) {
    ...
    this.streamPipes = new RustStreamPipeEmitter(context, this.streamPromises);
  }
  ...
  emitLibCall(expr: RustLibCallExpr): string | null {
    switch (expr.fn) {
      ...
      default: return this.streamPipes.emitLibCall(expr) ??
        this.transform.emitLibCall(expr) ?? this.duplex.emitLibCall(expr) ??
