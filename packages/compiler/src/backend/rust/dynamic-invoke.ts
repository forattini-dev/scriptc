...
class RustDynamicInvokeEmitter {
  ...
  private emitArrayArm(): void {
    ...
    this.context.line(`"splice" if args.len() <= 2 => { let start = sc_dyn_index_arg(args, 0, 0.0, callee_name); let delete_count = if args.len() < 2 { f64::INFINITY } else { sc_dyn_index_arg(args, 1, 0.0, callee_name) }; ${this.dyn}::Array(runtime::array_splice(array, start, delete_count)) },`);
