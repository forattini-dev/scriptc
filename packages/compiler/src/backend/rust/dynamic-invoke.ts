...
class RustDynamicInvokeEmitter {
  ...
  private emitArrayArm(): void {
    ...
    this.context.line(`"splice" => { let start = sc_dyn_index_arg(args, 0, 0.0, callee_name); let delete_count = if args.len() < 2 { f64::INFINITY } else { sc_dyn_index_arg(args, 1, 0.0, callee_name) }; ${this.dyn}::Array(runtime::array_splice_with_items(array, start, delete_count, args.get(2..).unwrap_or(&[]).to_vec())) },`);
    ...
    this.context.line("\"keys\" | \"values\" | \"entries\" | \"toReversed\" | \"toSorted\" | \"toSpliced\" | \"with\" | \"toString\" | \"toLocaleString\" => runtime::throw_error(format!(\"'Array.prototype.{method}' on a dynamic value is not supported yet\")),");
    this.context.line("_ => runtime::throw_type_error(format!(\"{callee_name} is not a function\")),");
    this.close("}");
    this.close("},");
  }
