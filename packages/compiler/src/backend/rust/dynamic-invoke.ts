...
class RustDynamicInvokeEmitter {
  ...
  private emitAbortSignalArm(): void {
    ...
    this.context.line(`"addEventListener" => { let event = match args.first() { Some(${this.dyn}::String(value)) => value.as_ref(), value => sc_dyn_arg_type_fail("type", "of type string", value.unwrap_or(&${this.dyn}::Undefined)), }; let callback = args.get(1).cloned().unwrap_or(${this.dyn}::Undefined); let identity = sc_dyn_function_identity(&callback).unwrap_or_else(|| sc_dyn_arg_type_fail("listener", "of type function", &callback)); if event == "abort" { let traced = callback.clone(); runtime::abort_signal_add_listener(signal, identity, std::rc::Rc::new(move |signal| { let event = runtime::map_new(); runtime::map_set_by(&event, runtime::string("type"), ${this.dyn}::String(runtime::string("abort")), |left, right| left.as_ref() == right.as_ref()); runtime::map_set_by(&event, runtime::string("target"), ${this.dyn}::AbortSignal(signal.clone()), |left, right| left.as_ref() == right.as_ref()); let _this_guard = sc_dyn_this_push(${this.dyn}::AbortSignal(signal.clone())); let _ = sc_dyn_call(&callback, &[${this.dyn}::Object(event)], "listener"); }), std::rc::Rc::new(move |tracer| runtime::Trace::trace(&traced, tracer))); } ${this.dyn}::Undefined },`);
