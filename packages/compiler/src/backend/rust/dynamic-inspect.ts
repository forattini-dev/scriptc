...
export function emitRustDynamicInspect(
  ...
  context.line("let module_namespace = runtime::map_is_module_namespace(object);");
  context.line("let keys = runtime::map_string_keys_js_order(object);");
  context.line("let length = runtime::array_len(&keys);");
  context.line("if length == 0.0 { return runtime::string(if module_namespace { \"[Module: null prototype] {}\" } else if null_proto { \"[Object: null prototype] {}\" } else { \"{}\" }); }");
  context.line("if recurse > depth { return runtime::string(if module_namespace { \"[Module]\" } else if null_proto { \"[Object: null prototype]\" } else { \"[Object]\" }); }");
  context.line("runtime::inspect_begin(recurse + 1.0);");
  context.line("let mut index = 0.0;");
  context.line("while index < length {");
  context.pushIndent();
  context.line("let key = runtime::array_get(&keys, index);");
  context.line("let field = if module_namespace { sc_dyn_object_key_get(object, &key, value) } else { runtime::map_get_by(object, &key, |left, right| left.as_ref() == right.as_ref()).expect(\"scriptc: missing dynamic object field\") };");
  ...
  context.popIndent();
  context.line("}");
  context.line("let base = if module_namespace { runtime::string(\"[Module: null prototype]\") } else if null_proto { runtime::string(\"[Object: null prototype]\") } else { runtime::empty_string() }; runtime::inspect_end(&base, &runtime::string(\"{\"), &runtime::string(\"}\"), recurse + 1.0, false, false)");
  context.popIndent();
  context.line("},");
