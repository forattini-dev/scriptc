    ...
    switch (expr.kind) {
      ...
      case "libCall":
        return emitRustLibCall(expr, {
          ...
          stripCasts: (value) => this.stripCasts(value),
          hasClassMeta: (name) => this.classMeta.has(name),
          classFieldName: (className, fieldName, loc) => this.classFieldName(className, fieldName, loc),
          ...
          emitFileHandleTransferPromise: (value) => this.emitFileHandleTransferPromise(value),
          emitFsRenameCallback: (value) => this.emitFsRenameCallback(value),
          emitClosureDispatch: (callee, type, args, loc) => this.emitClosureDispatch(callee, type, args, loc),
          classNameArms: (className, loc) => {
            const meta = this.classMetaOf(className, loc);
            return this.classSubtree(meta).map((candidate) =>
