...
export class RustDefinitionEmitter {
  ...
  emitErrorValueDefinition(): void {
    ...
    this.context.popIndent();
    this.context.line("}");
    this.context.line(`fn sc_error_identity(value: &${name}) -> usize {`);
    this.context.pushIndent();
    this.context.line("match value {");
    this.context.pushIndent();
    this.context.line(`${name}::Builtin(error) => runtime::error_identity(error),`);
    for (const root of roots) this.context.line(`${name}::${this.context.errorValueVariant(root)}(value) => value.identity(),`);
    this.context.popIndent();
    this.context.line("}");
    this.context.popIndent();
    this.context.line("}");
    this.context.line(`fn sc_error_code(value: &${name}) -> Option<runtime::JsString> {`);
    this.context.pushIndent();
    ...
    this.context.popIndent();
    this.context.line("}");
    this.context.line(`fn sc_error_dom_cause<T: Clone + 'static>(value: &${name}) -> Option<T> {`);
    this.context.pushIndent();
    this.context.line(`match value { ${name}::Builtin(error) => runtime::error_dom_cause(error), _ => None, }`);
    this.context.popIndent();
    this.context.line("}");
