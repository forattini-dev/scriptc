...
/** Recognize structural projections before choosing shared or copied storage. */
export function structuralProjection(fn: IrFunction) {
...
export function sharedWidthPair(fn: IrFunction, records: ReadonlyMap<string, IrRecordShape>, unions: ReadonlyMap<string, IrUnionDef>) {
  ...
  const { source, target } = projection;
