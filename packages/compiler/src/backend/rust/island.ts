  ...
  if ((expr.op === "eq" || expr.op === "neq") && expr.args.length === 2) {
    ...
    const equal = context.hasEmbeddedModules()
      ? `match (&${left}, &${right}) { ` +
        ...
        `(${dyn}::Island(sc_value), ${dyn}::Number(sc_other)) | (${dyn}::Number(sc_other), ${dyn}::Island(sc_value)) => runtime::island_strict_equal_number(sc_value, *sc_other), ` +
        `(${dyn}::Island(sc_value), ${dyn}::String(sc_other)) | (${dyn}::String(sc_other), ${dyn}::Island(sc_value)) => runtime::island_strict_equal_string(sc_value, sc_other), ` +
