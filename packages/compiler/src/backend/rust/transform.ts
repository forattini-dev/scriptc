...
export class RustTransformEmitter {
  ...
  private emitNewDynamic(expr: RustLibCallExpr): string {
    ...
    const passthrough = expr.fn === "passthrough.newDyn";
    const expectedClass = passthrough ? "%PassThrough" : "%Transform";
    if (options?.type.kind !== "dyn" || expr.args.length !== 1 ||
      expr.type.kind !== "object" || expr.type.className !== expectedClass) {
      this.context.unsupported("dynamic Transform constructor shape", expr.loc);
    }
