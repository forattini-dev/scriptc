    ...
    switch (e.kind) {
      ...
      case "awaitUnionExpr": {
        ...
        if (inner.kind === "union" && typeEquals(e.type, inner)) {
          const result = host.unionsById.get(inner.unionId);
          ...
          plainTags.forEach(({ arm }, i) => {
            ...
            let wrapped: string;
            if (isUnitType(arm)) {
              wrapped = host.unitInstanceRef(inner.unionId, targetTag);
            } else {
              const payload = host.unionExtract(u.name, arm);
              wrapped = host.unionNewOwned(targetTag, { name: payload, type: arm });
            }
          ...
          const out = host.own({ name: value, type: e.type });
          host.emitPendingCheck();
          return out;
        }
        ...
        if (typeEquals(e.type, inner)) {
          ...
          const plain = host.unionExtract(u.name, inner);
