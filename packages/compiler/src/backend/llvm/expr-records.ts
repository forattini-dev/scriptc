...
export function emitRegexIntrinsic(host: LlvmEmitterContext, e: IrExpr & { kind: "regexIntrinsic" }): LlValue {
    ...
    const capture = regexCaptureLayout(e.type, (id) => host.unionsById.get(id));
    const captureArgs = capture ? `, i32 ${capture.stringTag}, ptr ${host.unitInstanceRef(capture.id, capture.undefinedTag)}` : "";
    const fallible = (sym: string, argText: string): LlValue => {
      host.declare(`declare ptr @${sym}(${argText.split(", ").map((arg) => arg.split(" ")[0]).join(", ")})`);
      const t = B.tmp();
    ...
    switch (method) {
      ...
      case "match": {
        ...
        host.declare(`declare ptr @scr_regex_match(ptr, ptr, i32, ptr)`);
        const raw = B.tmp();
