    ...
    switch (e.kind) {
      ...
      case "nullish": {
        ...
        if (typeEquals(e.type, e.left.type)) {
          ...
          const result = host.unionsById.get(e.type.unionId);
          ...
          host.unionTagSwitch(l.name, def, (arm) => {
            ...
            const extracted = { name: host.unionExtract(l.name, arm), type: arm };
            const wrapped = host.unionNewOwned(resultTag, extracted);
