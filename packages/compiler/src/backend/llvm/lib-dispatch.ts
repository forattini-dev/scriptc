...
export function emitIoLibCall(host: LlvmEmitterContext, e: LibCallExpr): LlValue {
    ...
    if (e.fn === "rl.create") {
      ...
      const output = host.emitExpr(e.args[0]!);
      host.declare(`declare double @scr_rl_create_with_output(i1)`);
      const t = B.tmp();
...
export function emitLibCall(host: LlvmEmitterContext, e: LibCallExpr): LlValue {
    ...
    if (e.fn === "dyn.errInstanceof") {
      ...
      const target = host.classMetaOf(e.args[1].value);
      host.declare("declare zeroext i1 @scr_dyn_err_instanceof(ptr, double, double)");
      const out = B.tmp();
