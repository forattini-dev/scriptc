...
import { IrExpr, IrType, MAY_THROW_STR_METHODS, isRefCounted, typeEquals, typeKey } from "../../ir/ir.js";
...
export function emitStrIntrinsic(host: LlvmEmitterContext, e: IrExpr & { kind: "strIntrinsic" }): LlValue {
    ...
    switch (method) {
      ...
      case "at": {
        ...
        if (MAY_THROW_STR_METHODS.has(method)) host.emitPendingCheck();
        return out;
      }
      case "indexOf":
        return call(
...
export function emitMapNew(host: LlvmEmitterContext, e: IrExpr & { kind: "mapNew" }): LlValue {
    ...
    if (mapKeyAccess(e.type.key) === "ref") {
      ...
      host.declare(`declare ptr @scr_map_new_ref_key(i32, ptr, ptr, ptr, ptr, ptr, ptr)`);
      B.line(
        `${m} = call ptr @scr_map_new_ref_key(i32 ${mapValKindNum(value)}, ptr ${rc.retain}, ptr ${rc.release}, ptr ${isRefCounted(value) ? traceArg(host, value) : "null"}, ptr ${keyRc.retain}, ptr ${keyRc.release}, ptr ${traceArg(host, e.type.key)})`,
      );
    } else {
...
export function emitSetNew(host: LlvmEmitterContext, e: IrExpr & { kind: "setNew" }): LlValue {
    ...
    if (kAcc === "ref") {
      ...
      host.declare(`declare ptr @scr_set_new_ref(ptr, ptr, ptr)`);
      B.line(`${s} = call ptr @scr_set_new_ref(ptr ${rc.retain}, ptr ${rc.release}, ptr ${traceArg(host, e.type.elem)})`);
    } else {
      host.declare(`declare ptr @scr_map_new(i32, i32, ptr, ptr, ptr)`);
