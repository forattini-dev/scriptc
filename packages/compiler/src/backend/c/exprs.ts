...
import { bytesOf, canMarshalFuncIntoIsland, CHILDSTREAM_T, DYN, F64, IrExpr, IrLibFn, IrRecordShape, IrType, islandPromisePayloadTag, isFfiCallbackParam, isFfiContextParam, isFfiReleaseParam, isRefCounted, isUnitType, MAY_THROW_LIB_FNS, MAY_THROW_STR_METHODS, RUNTIME_ERROR_CLASSES, STRING, typeEquals, typeKey } from "../../ir/ir.js";
  ...
  switch (method) {
    case "get": {
      ...
      if (value.kind === "dyn" && e.type.kind === "dyn") {
        ...
        const result = emitter.newTemp(e.type, raw);
        emitter.line(`if (!${result.name}) ${result.name} = scr_dyn_retain(scr_dyn_undefined());`);
        return result;
      }
          ...
          def.arms.forEach((arm, sourceTag) => {
            ...
            const wrapped = arm.kind === "f64"
              ...
              : arm.kind === "bool"
                ...
                : (() => {
                    const rc = vAdapters(arm);
                    return `scr_union_new_ref(${resultTag}, ${retainCallC(arm, read)}, &${rc.retain}, &${rc.release}, ${emitter.traceArgC(arm)})`;
                  })();
            emitter.line(`case ${sourceTag}: ${name} = ${wrapped}; break;`);
          });
          emitter.line(`default: scr_trap("scriptc: internal error: invalid nullish union tag\\n");`);
          emitter.indent--;
          emitter.line(`}`);
          emitter.releaseValue(l.name, e.left.type);
        } else {
  ...
  switch (e.kind) {
      ...
      case "strIntrinsic": {
        ...
        switch (method) {
          ...
          case "codePointAt":
            return emitter.newTemp(e.type, `scr_str_code_point_at(${r.name}, ${args[0]!.name})`);
          case "charAt":
            return emitter.newTemp(e.type, `scr_str_char_at(${r.name}, ${args[0]!.name})`);
          case "at": {
            const out = emitter.newTemp(e.type, `scr_str_at(${r.name}, ${args[0]!.name})`);
            if (MAY_THROW_STR_METHODS.has(method)) emitter.emitPendingCheck();
            return out;
          }
          case "indexOf":
            return emitter.newTemp(
          ...
          case "startsWith":
            return emitter.newTemp(e.type, `scr_str_starts_with(${r.name}, ${args[0]!.name}, ${args[1]?.name ?? "0"})`);
          case "endsWith":
            return emitter.newTemp(e.type, `scr_str_ends_with(${r.name}, ${args[0]!.name}, ${args[1]?.name ?? "INFINITY"})`);
          case "slice":
            return emitter.newTemp(
              e.type,
      ...
      case "regexIntrinsic": {
        ...
        switch (method) {
          ...
          case "match": {
            ...
            const m = emitter.newTemp(arrayType, `scr_regex_match(${r.name}, ${args[0]!.name}${captureArgs})`);
          ...
          case "matchAll":
            ...
            return emitter.fallibleTemp(e.type, `scr_regex_match_all(${r.name}, ${args[0]!.name}${captureArgs})`);
          case "matchAllInto":
  ...
  switch (e.kind) {
        ...
        const m = emitter.newTemp(
          ...
          keyRc
            ? `scr_map_new_ref_key(${mapValKindC(value)}, ` +
              ...
              `&${keyRc.retain}, &${keyRc.release}, ${emitter.traceArgC(e.type.key)})`
      ...
      case "setNew": {
        ...
        const s = emitter.newTemp(
          e.type,
          rcAdapters
            ? `scr_set_new_ref(&${rcAdapters.retain}, &${rcAdapters.release}, ${emitter.traceArgC(e.type.elem)})`
...
function emitDynamicExpr(
  ...
  switch (e.kind) {
        ...
        const arrTest =
          e.key === "length"
            ...
            : /^(0|[1-9][0-9]*)$/.test(e.key) && Number(e.key) <= Number.MAX_SAFE_INTEGER
              ...
              : "false";
        const test = `(${d.name}->kind == SCR_DYN_OBJ ? (${objTest}) : ${d.name}->kind == SCR_DYN_ARR ? (${arrTest}) : scr_dyn_has_key(${d.name}, (ScrStr *)&${emitter.internLiteral(e.key)}))`;
      ...
      case "dynTest": { if (e.test === "bigint" || e.test === "date" || e.test === "symbol") throw new Error("native BigInt/Date/Symbol tags require --backend rust");
  ...
  switch (e.kind) {
      ...
      case "awaitUnionExpr": {
        ...
        if (inner.kind === "union" && typeEquals(e.type, inner)) {
          const result = emitter.unionsById.get(inner.unionId);
          ...
          const name = `sc_t${emitter.tempCounter++}`;
          emitter.line(`${cDecl(inner, name)} = NULL;${emitter.srcComment(e.loc)}`);
          emitter.currentFrame().push({ name, type: inner });
          emitter.line(`if (${u.name}->tag == ${e.promiseTag}) {`);
          emitter.indent++;
          emitter.line(`${name} = (ScrUnion *)scr_await_ref(${peek});`);
          emitter.indent--;
          emitter.line(`} else {`);
          emitter.indent++;
          ...
          def.arms.forEach((arm, tag) => {
            ...
            let wrapped: string;
            if (isUnitType(arm)) {
              wrapped = emitter.unitInstanceRef(inner.unionId, targetTag);
              ...
              wrapped = `scr_union_new_ref(${targetTag}, ${payload}, ${adapters.retain}, ${adapters.release}, ${emitter.traceArgC(arm)})`;
            } else {
              throw new InternalCompilerError(`emitter bug: collapsed awaitUnion source arm ${arm.kind}`);
            }
            emitter.line(`case ${tag}: ${name} = ${wrapped}; break;`);
          });
        ...
        if (typeEquals(e.type, inner)) {
          ...
          emitter.line(`${cDecl(inner, name)}${ref ? " = NULL" : ""};${emitter.srcComment(e.loc)}`);
          if (ref) emitter.currentFrame().push({ name, type: inner });
          let awaited: string;
          let plain: string;
          if (inner.kind === "f64") {
          ...
          emitter.line(`if (${u.name}->tag == ${e.promiseTag}) ${name} = ${awaited}; else { scr_await_hop(); ${name} = ${plain}; }`);
          emitter.emitPendingCheck();
          return { name, type: e.type };
        }
...
function emitDynamicLibCall(state: LibCallState): Temp {
  ...
  switch (fn) {
          ...
          case "dyn.errInstanceof":
            ...
            const dynErrorTarget = emitter.classMeta.get(e.args[1].value);
            if (!dynErrorTarget) {
...
function emitChildProcessLibCall(state: LibCallState): Temp {
  ...
  switch (fn) {
          ...
          case "child.onClose": {
            ...
            emitter.line(
              `scr_child_on_${e.fn === "child.onClose" ? "close" : "exit"}(${arg(0)}, ${cb.name}, &${adapter});${emitter.srcComment(e.loc)}`,
            );
            return { name: "", type: e.type };
          }
          case "child.onError": {
...
function emitProcessLibCall(state: LibCallState): Temp {
  ...
  switch (fn) {
          ...
          case "process.rows": {
            ...
            const w = emitter.newTemp(F64, `${runtimeFn}(${arg(0)})`);
...
function emitIoLibCall(state: LibCallState): Temp {
  ...
  switch (fn) {
          ...
          case "rl.nextLine":
            ...
            return emitReadlineNextLine(emitter, e, arg(0));
...
function emitLibCallExpr(emitter: CEmitter, e: LibCallExpr): Temp {
  ...
  switch (fn) {
          case "module.import": case "module.namespace": throw new Error("the C backend does not support native local module imports yet; use --backend rust");
          case "promise.view": throw new Error("the C backend does not support Promise payload views yet; use --backend rust");
