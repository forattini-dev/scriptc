...
export function isNumericCoercionFn(fn: IrLibFn): fn is IrNumericCoercionFn {
  return ["num.parseInt", "num.parseFloat", "num.fromString", "num.isNaN", "dyn.toStringCoerce", "dyn.stringConstructor", "dyn.toNumberCoerce", "dyn.compare"].includes(fn);
}
...
export function emitNumericCoercion(fn: IrNumericCoercionFn, args: readonly string[]): string {
  switch (fn) {
    ...
    case "dyn.toStringCoerce": case "dyn.stringConstructor": return `scr_dyn_string_coerce_js(${args[0]})`;
