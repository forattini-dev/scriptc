 ...
 * use-site trap (nodeThrowExpr) instead of a value. */
  export function trapModuleOf(lowerer: Lowerer, ident: ts.Identifier): { module: string; member: string } | null {
    const symbol = lowerer.checker.getSymbolAtLocation(ident);
    const decl = symbol ? lowerer.checker.declarationsOf(symbol)[0] : undefined;
    if (!decl) return null;
...
/** One builtin-module function call → its libCall. Completes the call to
  ...
  export function lowerBuiltinModuleCall(lowerer: Lowerer, expr: ts.CallExpression,
    ...
    if (bi.module === "fs/promises" && bi.member === "open") {
      ...
      const numericFlags = expr.arguments[1] && lowerer.mapTypeOf(lowerer.typeOf(expr.arguments[1]))?.kind === "f64";
      const flags = expr.arguments[1]
        ? numericFlags ? lowerer.lowerExprExpecting(expr.arguments[1], F64) : lowerBuiltinOptionalDefault(lowerer, expr.arguments[1], STRING, defaultFlags)
        : defaultFlags;
    ...
    if (bi.module === "fs/promises" && bi.member === "rm" && expr.arguments.length === 2) {
      const opts = literalBoolOptions(lowerer, expr.arguments[1]!, ["recursive", "force"]);
      if (opts === null) {
        lowerer.noLowering(
    ...
    if (isAccessSync || (bi.module === "fs/promises" && bi.member === "access")) {
      ...
      if (expr.arguments.length < 1 || expr.arguments.length > 2) {
        lowerer.noLowering(`${spelling} with ${expr.arguments.length} arguments`, expr);
      }
      const path = lowerer.lowerExprExpecting(expr.arguments[0]!, STRING);
...
/** True when `node`'s checker type is readline's Interface (stdlib
  ...
  export function isReadlineTyped(lowerer: Lowerer, node: ts.Expression): boolean {
...
export { lowerJsonMethodCall } from "./lower-json.js";
...
/** The composed crypto pattern: `randomBytes(n).toString(enc)` lowers
  ...
  export function lowerCryptoComposedCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (access.name.text === "digest") {
      return lowerHashDigestChain(lowerer, call, access) ?? lowerHmacDigestChain(lowerer, call, access);
    }
...
/** The composed hash chain — `createHash("sha256").update(data).digest("hex")`
  ...
  function lowerHashDigestChain(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (!algT?.isStringLiteralType() || !isLoweredDigestAlgorithm(algT.value)) {
      lowerer.noLowering("createHash with this algorithm", chCall, DIGEST_ALGORITHM_HINT);
    }
...
/** The composed HMAC chain —
  ...
  function lowerHmacDigestChain(lowerer: Lowerer, call: ts.CallExpression,
    access: ts.PropertyAccessExpression,): IrExpr | null {
    const updateCall = access.expression;
    if (!ts.isCallExpression(updateCall) || updateCall.questionDotToken) return null;
    const updAccess = updateCall.expression;
    ...
    const loc = locOf(call);
    const algT = chCall.arguments.length === 2 ? lowerer.typeOf(chCall.arguments[0]!) : undefined;
    if (!algT?.isStringLiteralType() || !isLoweredDigestAlgorithm(algT.value)) {
      lowerer.noLowering(
    ...
    const keyIr = lowerer.mapTypeOf(lowerer.typeOf(keyNode));
    let key: IrExpr;
    if (keyIr?.kind === "bytes" && keyIr.elem === "u8") {
      key = lowerer.lowerExprExpecting(keyNode, BYTES_U8);
    } else if (keyIr?.kind === "string") {
      // Node's own contract for a string key: its UTF-8 bytes.
      key = {
        kind: "libCall",
        fn: "buffer.fromStr",
        args: [lowerer.lowerExprExpecting(keyNode, STRING), strLit("utf8", loc)],
        type: BYTES_U8,
        loc,
      };
    } else {
      lowerer.noLowering(
        `createHmac key of '${keyIr ? lowerer.fmt(keyIr) : lowerer.checker.typeToString(lowerer.typeOf(keyNode))}' values`,
        keyNode,
    ...
    lowerer.noLowering(
      `Hmac.update of '${dataIr ? lowerer.fmt(dataIr) : lowerer.checker.typeToString(lowerer.typeOf(dataNode))}' values`,
      dataNode,
      "string and Buffer/Uint8Array inputs are the lowered update forms",
    );
  }
  ...
  export function lowerCryptoModuleCall(lowerer: Lowerer, expr: ts.CallExpression,
    ...
    if (bi.member === "timingSafeEqual") {
      ...
      const views = expr.arguments.length === 2
        ? expr.arguments.map((argument) => lowerer.mapTypeOf(lowerer.typeOf(argument)))
        : undefined;
      if (
        views === undefined ||
      ...
      return {
        ...
        args: expr.arguments.map((argument) => lowerer.lowerExprExpecting(argument, BYTES_U8)),
        type: BOOL,
        loc,
      };
    }
    if (bi.member === "hash") {
      ...
      const algorithmType = lowerer.typeOf(algorithmNode);
      ...
      if (dataType?.kind !== "string" &&
        ...
        lowerer.noLowering(
          `crypto.hash of '${dataType ? lowerer.fmt(dataType) : lowerer.checker.typeToString(lowerer.typeOf(dataNode))}' values`,
          dataNode,
          "string and Buffer/Uint8Array inputs are the lowered one-shot forms",
        );
      }
      const algorithm = lowerer.lowerExprExpecting(algorithmNode, STRING);
      ...
      if (encodingNode === undefined) {
        ...
        encoding = lowerer.lowerExprExpecting(encodingNode, STRING);
      }
      return {
        kind: "libCall",
...
/** Named fs.constants imports use destination-platform numeric values. */
  export function lowerFsConstantsProperty(lowerer: Lowerer, expr: ts.PropertyAccessExpression): IrExpr | null {
    ...
    const value = fsConstantValue(expr.name.text, lowerer.targetPlatform);
    if (value === undefined) {
      lowerer.noLowering(
...
/** Node's global Navigator facade. Its userAgent is tied to the runtime's
  ...
  export function lowerNavigatorProperty(lowerer: Lowerer, expr: ts.PropertyAccessExpression): IrExpr | null {
    if (lowerer.stdlibGlobalMember(expr, "navigator") !== "userAgent") return null;
    ...
    if (
      ...
      if (stream === "stdout" || stream === "stderr") {
        ...
        if (call.arguments.length !== 2 || eventArg === undefined || callbackArg === undefined) {
          lowerer.noLowering(`process.${stream}.${access.name.text} with ${call.arguments.length} arguments`, call);
        }
        const eventType = lowerer.typeOf(eventArg);
        ...
        const cb = lowerer.lowerExpr(callbackArg);
        if (cb.type.kind !== "func" || cb.type.params.length !== 0 || cb.type.ret.kind !== "void") {
          lowerer.unsupported("SC1090", callbackArg, "resize listeners must have the shape () => void");
        }
 ...
 * receivers. */
  export function lowerNumberStaticCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (member === "parseFloat" || member === "parseInt") {
      ...
      const lowered = call.arguments.map(arg => lowerAbsenceProbe(lowerer, arg) ?? lowerer.lowerExpr(arg));
      const parsed = lowerNumericParser(lowerer, member, lowered[0]!, lowered[1]);
      if (parsed) return parsed;
      lowerer.requireDynamicApi(`'Number.${member}'`, call);
      const callee: IrExpr = { kind: "jsOp", op: "globalGet", name: member, args: [], type: JSVAL, loc };
      const args = lowered.map((arg, index) => lowerer.jsvalIn(arg, call.arguments[index]!));
...
/** The Date slice: statics plus read-only Date values backed by one
  ...
  export function lowerDateCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (lowerer.stdlibGlobalMember(access, "Date") === "parse") {
      ...
      const value = lowerer.lowerExprExpecting(argNode, STRING);
