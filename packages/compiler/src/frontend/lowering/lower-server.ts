...
function lowerNetServerMethodCall(lowerer: Lowerer, call: ts.CallExpression,
  ...
  if (name === "listen") {
    ...
    if (ts.isObjectLiteralExpression(args[0]!)) {
      ...
      for (const prop of (args[0] as ts.ObjectLiteralExpression).properties) {
        ...
        if (key === "port") {
          ...
          lowerer.noLowering(
            ...
            prop,
            "abort-driven close has no lowering yet — port, host, ipv6Only, and reusePort are supported",
          );
        } else {
          lowerer.noLowering(
            `listen option '${key}'`,
      ...
      const optionsFn: IrLibFn = reusePort === null ? "net.listenOpts" : "net.listenOptsReusePort";
      const optionsCbFn: IrLibFn = reusePort === null ? "net.listenOptsCb" : "net.listenOptsReusePortCb";
      if (args.length === 1) {
        return listenResult(optionsFn, reusePort === null
      ...
      return listenResult(optionsCbFn, reusePort === null
...
function lowerHttpResMethodCall(lowerer: Lowerer, call: ts.CallExpression,
  ...
  if (name === "write" || name === "end") {
    ...
    if (name === "end" && args.length === 2) {
      ...
      const mapped = lowerer.mapTypeOf(probe);
      if (mapped?.kind === "dyn" || (probe.flags & ts.TypeFlags.Any) !== 0) return dynamicMethodInvoke(receiver, "end", "res.end", [lowerer.lowerExpr(args[0]!)], loc);
