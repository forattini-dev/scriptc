    ...
    mixin?: { base: ClassInfo; name: string; call: ts.CallExpression; bindings: Map<ts.Symbol, IrType>; context: string; ordinal: number },): void {
    {
      ...
      for (const clause of inst || mixin ? [] : (decl.heritageClauses ?? [])) {
        ...
        if (schema !== null) { // the kernel's heritage call, never a runtime base; error forms root at the runtime Error
          if (schema.form === "error" || schema.form === "taggedError") base = lowerer.classes.get("%Error") ?? null;
          continue;
        }
          ...
          for (const p of member.parameters) {
            ...
            if (inheritedType) {
              ...
              const readonlyOverride =
                ...
                typeEquals(lowerer.stripUndefinedArm(inheritedType), type);
              if (!readonlyOverride) {
      ...
      const schemaInfo = schema !== null ? collectSchemaClass(lowerer, decl, schema, ctor, fields, fieldOrder) : undefined;
      const ctorParams: ParamShape[] = schemaInfo !== undefined ? (schemaInfo.propsType !== null ? [{ type: schemaInfo.propsType, mode: "required" }] : [])
        : ctor && !mixinForwarding ? lowerer.paramShapes(ctor.parameters)
        : (base?.ctorParams ?? []);
...
/** PARAMETER-PROPERTY assignments (`this.x = x`, synthesized): run AFTER
  ...
  function paramPropInitStmts(lowerer: Lowerer, info: ClassInfo, thisLocal: IrLocal): IrStmt[] {
    ...
    for (const pp of info.paramProps ?? []) {
      ...
      value = lowerer.coerceInto(pp.param, value, pp.type);
...
function lowerProgramClassNew(lowerer: Lowerer, expr: ts.NewExpression, declaredInfo: ClassInfo, loc: SrcLoc): IrExpr {
  ...
  const args = info.schema !== undefined && info.schema.propsType === null ? [] // a propless schema class: `new X({})` carries nothing
    : lowerer.inheritsBuiltinErrorCtor(info)
...
export function lowerNew(lowerer: Lowerer, expr: ts.NewExpression): IrExpr {
    ...
    if (ts.isIdentifier(expr.expression)) {
      ...
      if (symbol && symbol.name === "Date" && lowerer.isStdlibSymbol(symbol)) {
        return lowerDateNew(lowerer, expr);
      }
      ...
      // Native arrays and collection seeds lower in lower-set-constructor;
      // unsupported element types and general iterable sources fence there.
      ...
      if (symbol?.name === "Set" && lowerer.isStdlibSymbol(symbol)) return lowerSetNew(lowerer, expr);
    ...
    // fence — annotate the slot with the common constructor type.
    {
      const storedCalleeT = ts.isIdentifier(expr.expression)
        ? (lowerer.resolveLocal(expr.expression) ?? lowerer.globalOf(expr.expression))?.type
        : undefined;
      const storedOptionalClass =
        storedCalleeT?.kind === "union" &&
        (() => {
          const arms = lowerer.unions.get(storedCalleeT.unionId)?.arms ?? [];
      ...
      const calleeT = storedOptionalClass
        ? storedCalleeT
        : (lowerer.mapTypeOf(lowerer.typeOf(expr.expression)) ?? storedCalleeT);
      ...
      if (calleeT?.kind === "union") {
        ...
        if (
          ...
          const info = lowerer.classes.get(classType.className);
          if (!info || info.generic) {
            lowerer.unsupported(
              "SC1090",
              expr,
          ...
          const unionValue = lowerer.lowerExpr(expr.expression);
          ...
          const callee: IrExpr = unionValue.type.kind === "classval"
            ...
            : {
                ...
                tag: lowerer.armTag(calleeT.unionId, classType),
                value: unionValue,
                type: classType,
                loc,
              };
          ...
          return {
            kind: "newValue",
            callee,
            args: lowerer.completeArgs(expr.arguments ?? [], info.ctorParams, loc, expr),
            type: { kind: "object", className: info.def.name },
            loc,
          };
        }
