...
export const BUILTIN_MODULE_FNS: Record<string, Record<string, BuiltinModuleFn | undefined> | undefined> = {
  ...
  "fs/promises": {
    ...
    // access's omitted mode completes to 0 (F_OK) in the special case
    // below, exactly like accessSync's; both share one runtime probe.
  ...
  os: {
    ...
    hostname: { fn: "os.hostname", params: [], result: STRING },
...
export { AMBIENT_SURFACE_FNS, type AmbientSurfaceRow } from "./ambient-surfaces.js";
  ...
  export function stdlibMemberFence(lowerer: Lowerer, access: ts.PropertyAccessExpression): void {
    ...
    if (recvIr?.kind === "promise" && (member === "then" || member === "catch" || member === "finally")) {
      ...
      hint =
        ...
        "including positional numbers before or after that spread";
    } else if (
      recvIr?.kind === "array" &&
      member === "index" &&
