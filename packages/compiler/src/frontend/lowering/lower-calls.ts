...
/** Nested `function name(...) {...}` reserves its binding first, then lowers
  ...
  export function lowerNestedFunctionDecl(L: Lowerer, stmt: ts.FunctionDeclaration): IrStmt {
    ...
    const local = L.declareLocal(stmt.name, stmt.name.text, funcType, true);
    const active = [...L.activeStmtLists].reverse().find((entry) => entry.stmts.includes(stmt));
...
/** `Object.keys(r)` / `Object.values(r)` / `Object.entries(r)` over FIXED
  ...
  function lowerObjectHasOwnArgs(L: Lowerer, call: ts.CallExpression,
  ...
  function lowerLegacyHasOwnCall(L: Lowerer, call: ts.CallExpression): IrExpr | null {
    ...
    if (!L.isStdlibGlobal(prototype.expression, "Object")) return null;
    return lowerObjectHasOwnArgs(L, call, call.arguments[0]!, call.arguments[1]!);
  }
