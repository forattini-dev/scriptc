...
/** One parameter's ParamShape — the shared signature-shaped collection
  ...
  export function paramShape(lowerer: Lowerer, param: ts.ParameterDeclaration): ParamShape {
    ...
    if (param.dotDotDotToken) {
      ...
      if (!isJsSourceFile(param.getSourceFile()) && isDynamicRestArray(lowerer, restTs)) {
...
/** Function values spell their completed ABI. Typed rests retain an array
 * slot and a rest marker; optional/defaulted slots require undefined arms.
 * A contextual assignment must agree with that convention. */
  export function requireExactArityValue(lowerer: Lowerer, blame: ts.Node,
    ...
    if (shapes.some((s) => s.mode === "rest") && !(funcType.kind === "func" && funcType.restAbi === "array")) {
    ...
    let target = contextual ? lowerer.checker.getContextualType(contextual) : undefined;
    ...
    if (
      ...
      const owner = lowerer.checker.getContextualType(blame.parent);
      const field = owner ? lowerer.checker.getPropertyOfType(owner, blame.name.text) : undefined;
      if (field) target = lowerer.checker.getTypeOfSymbolAtLocation(field, blame);
    }
    const mapped = target
    ...
    if (mapped?.kind === "dyn" &&
      canBoxFuncIntoDyn(funcType, (id) => lowerer.shapes.get(id), (id) => lowerer.unions.get(id))
    ) {
      return;
    }
    lowerer.unsupported(
...
/** A declaration's checker-derived IR return type. The unmappable-type
  ...
  export function declaredReturnType(lowerer: Lowerer, decl: ts.SignatureDeclaration, blame: ts.Node): IrType {
    ...
    const returnType = inferredRegexCaptureReturn(lowerer, decl, lowerer.mapTypeOf(retTsType));
    if (!returnType) {
...
/** The instance a CALL of a generic function-like names: resolved
  ...
  export function genericCallInstance(lowerer: Lowerer, expr: ts.CallExpression, info: GenericFnInfo): GenericInstance {
    ...
    if (selectedOverload) {
      ...
      lowerer.typeParamBindings = classBindings
        ? new Map([...classBindings, ...bindings])
        : bindings;
      lowerer.typeParamTsBindings = tsBindings;
      try {
        bindUnobservedOverloadTypeParams(lowerer, info, bindings, tsBindings);
        if (genericImplementationContractMaps(lowerer, info)) {
          const params = lowerer.paramShapes(info.decl.parameters);
          const nameBlame = (ts.isArrowFunction(info.decl) ? undefined : info.decl.name) ?? info.decl;
          const returnType = lowerer.declaredReturnType(info.decl, nameBlame);
          return internGenericInstance(lowerer, expr, info, params, returnType, () => bindings, {
            tsBindings,
          });
        }
        const params = resolvedGenericParamShapes(lowerer, expr, info, rsig);
        const retTs = lowerer.checker.getReturnTypeOfSignature(rsig);
        const returnType = lowerer.mapTypeOf(retTs);
        if (!returnType) lowerer.badType(expr, retTs);
        return internGenericInstance(lowerer, expr, info, params, returnType, () => bindings, {
          tsBindings,
        });
      } finally {
  ...
  function genericImplementationContractMaps(lowerer: Lowerer, info: GenericFnInfo): boolean {
    const sig = lowerer.checker.getSignatureFromDeclaration(info.decl);
    if (!sig) return false;
    const paramsMap = info.decl.parameters.every((param) => lowerer.mapTypeOf(lowerer.typeOf(param.name)) !== null);
    if (!paramsMap) return false;
    const ret = lowerer.checker.getReturnTypeOfSignature(sig);
    return (ret.flags & ts.TypeFlags.Never) !== 0 || lowerer.mapTypeOf(ret) !== null;
  }
  ...
  function resolvedGenericParamShapes(lowerer: Lowerer, expr: ts.CallExpression,
    ...
    rsig.getParameters().forEach((p, i) => {
      ...
      const pt = lowerer.checker.getTypeOfSymbol(p);
      if (
        declParam.dotDotDotToken &&
        !isJsSourceFile(info.decl.getSourceFile()) &&
        isDynamicRestArray(lowerer, pt)
      ) {
        params.push({ type: DYN, mode: "dynRest" });
        return;
      }
      const mapped = lowerer.mapTypeOf(pt);
      if (!mapped || mapped.kind === "void") lowerer.badType(expr.arguments[i] ?? expr, pt);
      if (declParam.dotDotDotToken) {
        if (mapped.kind !== "array") lowerer.badType(expr.arguments[i] ?? expr, pt);
        ...
        if (mapped.kind === "dyn" || mapped.kind === "jsval") {
          ...
          const bodyType = lowerer.stripUndefinedArm(mapped);
          lowerer.checkDefaultParamBodyType(declParam, bodyType);
          params.push({ type: lowerer.withUndefinedArm(bodyType), mode: "omittable", bodyType });
        }
      } else {
        if (declParam.questionToken && !lowerer.bareUndefinedArmedUnion(mapped)) {
          lowerer.unsupported("SC1090", declParam, `optional parameters of type '${lowerer.fmt(mapped)}'`);
...
/** The one instance table both instantiation routes share: key identity IS
   ...
   * costs checker walks). */
  export function internGenericInstance(lowerer: Lowerer, blame: ts.Node,
    info: GenericFnInfo,
    params: ParamShape[],
    returnType: IrType,
 ...
 * overload's result still crosses reconcileOverloadReturn. */
  function bindUnobservedOverloadTypeParams(lowerer: Lowerer, info: GenericFnInfo,
    ...
    info.typeParams.forEach((tp, i) => {
      ...
      const constraint = constraintNode
        ? lowerer.checker.getTypeFromTypeNode(constraintNode)
        : undefined;
      const mapped = constraint ? lowerer.mapTypeOf(constraint) : null;
      const fallback = mapped && mapped.kind !== "void" ? mapped : DYN;
      bindings.set(tp, fallback);
      lowerer.typeParamBindings?.set(tp, fallback);
...
/** Lowers ONE monomorphic instance of a generic function: the same body
  ...
  export function lowerGenericInstance(lowerer: Lowerer, info: GenericFnInfo, inst: GenericInstance): IrFunction {
    ...
    fnCtx.isAsync = isAsync; if (info.family) prepareFamilyInstanceCtx(lowerer, info, fnCtx); // family implementations lower LIFTED: the captures were taken when the value was built (the enclosing frame was live then)
...
/** A generic function taken as a VALUE, monomorphized by flow. A function
  ...
  export function lowerGenericFnValue(lowerer: Lowerer, ref: ts.Expression, info: GenericFnInfo): IrExpr {
    ...
    if (info.implicitParams) {
      ...
      const funcType = functionAbi(inst.params, inst.returnType);
  ...
  export function implicitCallInstance(lowerer: Lowerer, call: ts.CallExpression, info: GenericFnInfo): GenericInstance {
    ...
    info.decl.parameters.forEach((param, i) => {
      ...
      if (arg && !ts.isSpreadElement(arg)) {
        ...
        if (bindableImplicitIr(mapped) && !hasHiddenOptionalIndex(lowerer, arg, mapped)) {
          bound = mapped;
          argTypes.set(sym, t);
        }
      }
...
export function lowerFfiCall(lowerer: Lowerer, expr: ts.CallExpression): IrExpr | null {
    ...
    if (lowerer.ffiBindingSymbols !== null) {
      ...
      if (!lowerer.ownsValidatedFfiSymbol(binding.name, symbol)) {
        if (lowerer.libraryCallbacks) {
          const declarations = lowerer.checker.declarationsOf(symbol);
...
export function lowerCall(lowerer: Lowerer, expr: ts.CallExpression): IrExpr {
    ...
    if (ts.isIdentifier(expr.expression)) {
      ...
      const effectPipe = lowerEffectCall(lowerer, expr, loc); if (effectPipe !== null) return effectPipe; // effect's `pipe(e, …)`
    }
    ...
    if (consoleMember !== null) {
      ...
      const args = expr.arguments.map((a) => {
        ...
        if (lowered.type.kind === "jsval") {
          if (nativeImportHandleType(lowerer, a)?.kind === "jsval") {
    ...
    if (
      ...
      (expr.expression.questionDotToken !== undefined || nsMemberIdentOf(lowerer, expr.expression) === null)
    ) {
    ...
    if (lowerer.isIslandExpr(expr.expression)) {
      ...
      const callee = lowerer.lowerExpr(expr.expression); if (callee.type.kind === "genericFunc") return lowerFamilyCall(lowerer, expr, callee); // a generic function VALUE: the body compiled for THIS instantiation (lower-families.ts)
...
/** The RUNTIME-ARITY spread call — `f(...args)`, the rest-forwarding idiom
  ...
  function lowerSpreadArgsCall(lowerer: Lowerer, expr: ts.CallExpression, callee: IrExpr, loc: SrcLoc): IrExpr | null {
    ...
    if (
      (!anyJsvalSpread || (lowerer.dynamic && callee.type.kind === "dyn")) &&
      (callee.type.kind === "dyn" || (callee.type.kind === "func" && canBoxFuncIntoDyn(callee.type, getR, getU))) &&
      spreadParts.every((p) =>
        p.v.type.kind === "dyn" ||
        (lowerer.dynamic && callee.type.kind === "dyn" && p.v.type.kind === "jsval") ||
...
export function lowerDynDispatchMethodCall(
  ...
  if (
    ...
    if (
      ...
      (lowerer.checker.getTypeAtLocation(param.name).flags & (ts.TypeFlags.Any | ts.TypeFlags.Never)) !== 0 &&
      !lowerer.chainNarrowedType.has(param.name)
    ) {
      callbackParam = param.name;
      lowerer.chainNarrowedType.set(callbackParam, lowerer.checker.getUnknownType());
    }
  }
  let args: IrExpr[];
  try {
    const predicate = method === "filter" && callback ? lowerer.lowerExpr(callback) : null;
    ...
    if (callbackParam) lowerer.chainNarrowedType.delete(callbackParam);
  }
  return {
    kind: "dynInvoke",
    recv,
  ...
  function lowerPrimitiveProtoCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if ((name === "toPrecision" || name === "toString") && recvKind === "f64" &&
      ...
      if (argument.type.kind === "union") {
        const def = lowerer.unions.get(argument.type.unionId);
        ...
        if (def?.arms.length === 2 && numberTag >= 0 && undefinedTag >= 0) {
          const receiverLocal = lowerer.declareHiddenLocal("%numfmt.recv", F64);
          const argumentLocal = lowerer.declareHiddenLocal("%numfmt.arg", argument.type);
          const receiverRef: IrExpr = {
            kind: "varRef",
            localId: receiverLocal.id,
            type: F64,
  ...
  export function lambdaSignature(lowerer: Lowerer, node: ts.ArrowFunction | ts.FunctionExpression | ts.FunctionDeclaration | ts.MethodDeclaration | ts.GetAccessorDeclaration | ts.SetAccessorDeclaration,): { shapes: ParamShape[]; funcType: IrType & { kind: "func" } } {
    ...
    if (
      ...
      if (isAsyncLike && ctxRetRaw?.kind === "union") {
        const arms = lowerer.unions.get(ctxRetRaw.unionId)?.arms ?? [];
          ...
          arms.every(
            (arm) =>
              ...
              (promises[0]!.inner.kind === "void"
                ? arm.kind === "undefinedT"
                : lowerer.coercibleValue(arm, promises[0]!.inner) ||
                  (promises[0]!.inner.kind === "union" &&
                    lowerer.armTag(promises[0]!.inner.unionId, arm) >= 0)),
          )
        ) {
          ret = promises[0]!;
    ...
    return {
      shapes,
      funcType: functionAbi(shapes, ret, usesArguments),
    };
  }
...
/** Lifts an arrow function / function expression / nested declaration /
  ...
  export function lowerLambda(lowerer: Lowerer, node: ts.ArrowFunction | ts.FunctionExpression | ts.FunctionDeclaration | ts.MethodDeclaration | ts.GetAccessorDeclaration | ts.SetAccessorDeclaration,): IrExpr {
    const loc = locOf(node); if ((ts.isArrowFunction(node) || ts.isFunctionExpression(node)) && (node.typeParameters?.length ?? 0) > 0) return lowerFamilyImpl(lowerer, node, null); // a GENERIC arrow or function expression has no single signature to compile: it is one implementation of a closure family, compiled once per instantiation its call sites demand (lower-families.ts)
...
/** Marks an INLINE then-handler's unannotated identifier parameters for
  ...
  function lowerInlinePromiseRejectionHandler(
    ...
    if (handlerNode.parameters.length > 1) {
      lowerer.unsupported("SC1090", handlerNode, "rejection handlers with more than one parameter");
    }
  ...
  function appendPromiseHandlerResult(
    ...
    if (resultType.kind === "void") {
      ...
      body.push({ kind: "return", value: lowerer.coerceInto(blame, awaited, resultType), loc });
    } else {
      body.push({ kind: "return", value: lowerer.coerceInto(blame, handlerCall, resultType), loc });
    }
  }
...
export function lowerPromiseMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    let recvT = nativeImportHandleType(lowerer, access.expression) ?? lowerer.mapTypeOf(lowerer.typeOf(access.expression));
    ...
    if (thenPair) {
      ...
      const fulfilled = lowerer.lowerExpr(call.arguments[0]!);
      ...
      if (fulfilledParam !== undefined && !typeEquals(fulfilledParam, inner)) {
        lowerer.unsupported(
          ...
          `then fulfillment handlers whose parameter is not the settled value's type ` +
            `(expected '${lowerer.fmt(inner)}', got '${lowerer.fmt(fulfilledParam)}')`,
        );
      }
      ...
      const fnName = `%fn${lowerer.lambdaCounter++}_thenpair`;
      ...
      try {
        ...
        if (fulfilledParam !== undefined && inner.kind !== "void") {
          const valueLocal = lowerer.declareHiddenLocal("value", inner);
              ...
              : inner.kind === "string"
                ...
                : inner.kind === "bool"
                  ...
                  : inner.kind === "dyn"
                    ...
                    : lowerer.unassignedSlotInit(inner, loc);
          if (scratch === null) {
            lowerer.unsupported(
              "SC1090",
              call,
              `two-handler then over '${lowerer.fmt(inner)}' settled values ` +
        ...
        // onRejected handler.
        appendPromiseHandlerResult(lowerer, body, fulfilledCall, resultType, call, loc);
        const ctx = lowerer.ctx;
        const lifted: IrFunction = {
          name: fnName,
          params: [
    ...
    if (member === "then") {
      ...
      try {
        ...
        appendPromiseHandlerResult(lowerer, body, handlerCall, R, call, loc);
        const ctx = lowerer.ctx;
        const lifted: IrFunction = {
          name: fnName,
          params: [
    ...
    try {
      ...
      const rejection = lowerInlinePromiseRejectionHandler(lowerer, call.arguments[0]!, R, loc);
      const ctx = lowerer.ctx;
      const lifted: IrFunction = {
        name: fnName,
        params: [{ localId: pLocal.id, name: pLocal.name, type: promT }],
    ...
    const visit = (n: ts.Node): void => {
      ...
      if (ts.isIdentifier(n) && n.text === symText) {
        ...
        if (
          ...
          if (!ts.isExpressionStatement(p.parent) || !sideEffectFreeValueExpr(lowerer, p.right)) dead = false;
          return;
        }
  ...
  export function lowerObjectMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (info && !found) {
      ...
      if (fieldType && (fieldType.kind === "func" || fieldType.kind === "dyn")) {
        ...
        if (callee?.type.kind === "func") {
          const args = completeFunctionValueArgs(lowerer, call, callee.type);
    ...
    const receiverAs = (className: string): IrExpr =>
      receiver.type.kind === "object" && lowerer.isSubclassOf(className, receiver.type.className)
        ? {
            kind: "downcast",
            value: receiver,
        ...
        : lowerer.upcastTo(receiver, className);
