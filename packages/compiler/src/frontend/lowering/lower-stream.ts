...
import { streamArgumentTypeError } from "./stream-argument-validation.js";
...
function lowerStreamArg(lowerer: Lowerer, node: ts.Expression, what: "finished" | "pipeline"): IrExpr {
  ...
  if (!streamSidesOf(lowerer, info)) {
    const argumentError = streamArgumentTypeError(lowerer, node, v, what);
    if (argumentError) throw new StreamArgTypeThrow(argumentError);
    lowerer.noLowering(
