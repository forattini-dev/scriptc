...
export function lowerStmt(lowerer: Lowerer, stmt: ts.Statement): IrStmt | IrStmt[] | null {
    ...
    if (ts.isIfStatement(stmt)) {
      ...
      const else_ = stmt.elseStatement
        ...
        : null;
      const narrowedAfter = guardedExitRuntimeOptionalLocal(lowerer, stmt);
...
/** The stored-drain interception behind lowerVarStatement: lowers
  ...
  function lowerMatchAllDrainDecl(lowerer: Lowerer, decl: ts.VariableDeclaration, isConst: boolean): IrStmt[] | null {
    ...
    const rowsT = arrayOf(regexCaptureArray(lowerer.unions));
    ...
    if (ts.isArrayBindingPattern(pattern)) {
      ...
      pattern.elements.forEach((el, i) => {
        ...
        if (el.initializer) {
          ...
          value = arrayPositionOptionalValue(lowerer, el.name, value, srcRef, i, srcType.elem);
        }
        lowerer.bindPatternTarget(el.name, value, isLet, out);
      });
      return;
...
/** The array-position default core (the bounds test + undefined-arm
  ...
  function arrayPositionOptionalValue(
    ...
    const loc = read.loc;
    const optionalT = lowerer.withUndefinedArmOf(elemT);
    if (!optionalT || optionalT.kind !== "union") lowerer.badType(blame, lowerer.typeOf(blame));
    const source = srcRef();
    const length: IrExpr = {
      kind: "arrIntrinsic",
      method: "length",
    ...
    const absent = lowerer.wrappedUndefined(optionalT, loc);
    if (!absent) lowerer.badType(blame, lowerer.typeOf(blame));
    return {
      kind: "ternary",
      cond: inRange,
      then: lowerer.coerceInto(blame, read, optionalT),
      else_: absent,
      type: optionalT,
      loc,
    };
...
/** The tuple-tail packing core, shared with destructuring ASSIGNMENT
  ...
  function tupleTailValue(lowerer: Lowerer, blame: ts.Node,
    ...
    const tail = lowerNativeTupleTail(lowerer, srcRef(), from, restT, blame);
    if (tail) return tail;
    const reads: IrExpr[] = [];
...
/** The fresh record for `{ a, ...rest }` over a RECORD source: the
  ...
  function objectRestValue(lowerer: Lowerer, el: ts.BindingElement,
    ...
    const loc = locOf(el);
    let restT = patternBindingType(lowerer, el.name!); // callers skip nameless elisions
    ...
    if (restT === null && lowerer.typeParamBindings !== null) {
      ...
      restT = {
        kind: "record",
        shapeId: lowerer.shapes.intern(rem.map((f) => ({ name: f.name, type: f.type }))),
      };
    }
  ...
  export function bindPatternTarget(lowerer: Lowerer, name: ts.BindingName,
    ...
    if (ts.isIdentifier(name)) {
      ...
      const genericRecordRest =
        ...
        lowerer.typeParamBindings !== null &&
        ts.isBindingElement(name.parent) &&
        name.parent.dotDotDotToken !== undefined;
      let type = dynStays ? DYN : genericRecordRest ? value.type : lowerer.runtimeOptionalBindingType(name, lowerer.irTypeOf(name));
...
export function lowerVarDecl(lowerer: Lowerer, decl: ts.VariableDeclaration, isLet: boolean): IrStmt | null {
    ...
    try {
      init = immediatelyGuardedAbsenceProbe(lowerer, decl)
        ...
        : (lowerLocalGenericInitializer(lowerer, decl) ?? lowerVariableInitializer(lowerer, decl.initializer));
    } catch (e) {
    ...
    const bindingTainted = neverTaintedJsType(lowerer, decl.name, lowerer.typeOf(decl.name)) || jsArrayInferenceBinding(decl.name, lowerer.checker);
    const optionalClassValue =
      init.type.kind === "union" &&
      (() => {
        const arms = lowerer.unions.get(init.type.unionId)?.arms ?? [];
    ...
    let type = nativeProxyBindingType(lowerer, decl, init) ??
      ...
      ((init.type.kind === "jsval" ||
        ...
        ? nativeImportHandleType(lowerer, decl.initializer)
        : null) ??
      (lowerer.dynamic &&
      (init.type.kind === "jsval" ||
        (init.type.kind === "promise" && init.type.inner.kind === "jsval"))
    ...
    if (type?.kind === "array") type = inferredRegexBindingType(lowerer, decl, type);
    let runtimeOptional = false;
    if (
  ...
  function immediatelyGuardedAbsenceProbe(lowerer: Lowerer, decl: ts.VariableDeclaration): boolean {
    ...
    const guarded = strictUndefinedEqualityIdentifier(lowerer, condition);
    return guarded !== null && lowerer.resolveValueSymbol(guarded) === lowerer.resolveValueSymbol(decl.name);
  }

  function guardedExitRuntimeOptionalLocal(lowerer: Lowerer, stmt: ts.IfStatement): IrLocal | null {
    if (stmt.elseStatement) return null;
    let condition = stmt.expression;
    while (ts.isParenthesizedExpression(condition)) condition = condition.expression;
    const guarded = exitingAbsenceGuardIdentifier(lowerer, condition);
    if (guarded === null) return null;
    const local = lowerer.resolveLocal(guarded);
  ...
  function exitingAbsenceGuardIdentifier(lowerer: Lowerer, node: ts.Expression): ts.Identifier | null {
    let expr = node;
    ...
    const strict = strictUndefinedEqualityIdentifier(lowerer, expr);
    if (strict !== null) return strict;
    if (ts.isBinaryExpression(expr) && expr.operatorToken.kind === ts.SyntaxKind.BarBarToken) {
      return exitingAbsenceGuardIdentifier(lowerer, expr.left) ?? exitingAbsenceGuardIdentifier(lowerer, expr.right);
    }
    return null;
  }

  function strictUndefinedEqualityIdentifier(lowerer: Lowerer, node: ts.Expression): ts.Identifier | null {
    let expr = node;
    while (ts.isParenthesizedExpression(expr)) expr = expr.expression;
    if (
...
/** JS-exact switch (see docs/ir.md): one shared lexical scope for all case
  ...
  export function lowerSwitch(lowerer: Lowerer, stmt: ts.SwitchStatement): IrStmt {
    ...
    if (disc.type.kind === "jsval") {
      const mapped = lowerer.mapTypeOf(lowerer.typeOf(stmt.expression));
      if (mapped !== null && mapped.kind !== "jsval" && mapped.kind !== "dyn" && lowerer.boundaryExitSafe(mapped)) {
        const exited = lowerer.coerceToExpected(disc, mapped);
        const local = lowerer.declareLocal(stmt.expression, "%switch.disc", exited.type, false);
        ...
        return { kind: "block", body: [decl, lowerSwitchOn(lowerer, stmt, labels, ref)], loc: locOf(stmt) };
      }
    }
    return lowerSwitchOn(lowerer, stmt, labels, disc);
  }

  /** The switch proper, over an already-lowered discriminant. */
  function lowerSwitchOn(lowerer: Lowerer, stmt: ts.SwitchStatement, labels: string[] | undefined, disc: IrExpr): IrStmt {
    const dk = disc.type.kind;
    if (dk === "dyn") return lowerDynamicSwitch(lowerer, stmt, labels, disc);
...
/** Delete env/dictionary keys or clear an optional field (divergence 60).
  ...
  function lowerDeleteStatement(lowerer: Lowerer, expr: ts.DeleteExpression): IrStmt {
    ...
    const open = lowerOpenRecordDelete(lowerer, expr); if (open) return { kind: "exprStmt", expr: open, loc };
      ...
      if (opKind === ts.SyntaxKind.EqualsToken) {
        ...
        if (ts.isPropertyAccessExpression(expr.left)) {
          ...
          if (
            ...
            lowerer.stdlibGlobalMember(expr.left, "process") === "exitCode"
          ) {
            const loc = locOf(expr);
            const value = lowerer.lowerExpr(expr.right);
            if (value.type.kind !== "f64") {
              lowerer.unsupported("SC1090", expr.right, "non-number process.exitCode values");
            }
            return {
              kind: "exprStmt",
          ...
          const clear = lowerArrayClearAssignment(lowerer, expr.left, expr.right);
          ...
          if (expr.left.name.text === "pathname" && lowerer.mapTypeOf(lowerer.typeOf(expr.left.expression))?.kind === "url") {
            const loc = locOf(expr);
            const receiver = lowerer.lowerExpr(expr.left.expression);
            const value = lowerer.lowerExprExpecting(expr.right, STRING);
...
/** `for (const x of arr)` — arrays only. The lib types strings and Maps
  ...
  export function lowerForOf(lowerer: Lowerer, stmt: ts.ForOfStatement): IrStmt {
    ...
    if (stmt.awaitModifier) {
      ...
      if (isReadlineTyped(lowerer, stmt.expression)) {
        return lowerForAwaitReadline(lowerer, stmt);
      }
    ...
    if (iterable.type.kind !== "array") {
      ...
      iterable = lowerNativeTupleArrayView(lowerer, iterable) ?? iterable;
      if (
...
/** `for (const m of s.matchAll(re))` — the companion-index desugar. The
  ...
  function lowerForOfMatchAll(
    ...
    const loc = locOf(stmt);
    const rowT = regexCaptureArray(lowerer.unions);
...
/** `for await (const line of readlineInterface)` — the native readline
  ...
  function lowerForAwaitReadline(lowerer: Lowerer, stmt: ts.ForOfStatement): IrStmt {
    if (!lowerer.ctx.isAsync) {
      lowerer.unsupported("SC1090", stmt, "top-level 'for await' (await outside async functions)");
    }
    if (!ts.isVariableDeclarationList(stmt.initializer)) {
    ...
    if (!ts.isIdentifier(decl.name)) lowerer.unsupported("SC1031", decl.name);
    const loc = locOf(stmt);
    const lineT: IrType = { kind: "union", unionId: lowerer.unions.intern([STRING, UNDEFINED_T]) };
    ...
    return lowerer.env.inScope(() => {
      ...
      return {
        kind: "block",
        body: [
          { kind: "varDecl", localId: receiver.id, init: lowerer.lowerExpr(stmt.expression), loc },
          {
            kind: "while",
            cond: { kind: "boolLit", value: true, type: BOOL, loc },
            body: [
