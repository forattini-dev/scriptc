...
import { BOOL, BYTES_U8, CAUGHT, DYN, F64, IrBytesElem, IrBytesIntrinsicMethod, IrExpr, IrFunction, IrLocal, IrMapIntrinsicMethod, IrParam, IrSetIntrinsicMethod, IrStmt, IrType, JSVAL, STRING, SrcLoc, UNDEFINED_T, VOID, arrayOf, bytesOf, funcOf, isRefCounted, isSupportedArrayElem, isSupportedIndexValue, isUnitType, typeEquals } from "../../ir/nodes.js";
...
/** `a.map(fn)` / `a.filter(fn)` / `a.forEach(fn)` desugar to a direct
  ...
  function lowerArrayHofCall(L: Lowerer, call: ts.CallExpression,
    ...
    if (method === "map" && fnRet.kind === "dyn") {
      return lowerArrayMapDynamic(L, receiver, fnArg, elem, arity, loc);
    }
...
/** Read-only tuple methods snapshot fields into an array (the for-of-over-
  ...
  export function lowerTupleReadMethodCall(L: Lowerer, call: ts.CallExpression,
    ...
    if (fnRet.kind === "dyn") return lowerArrayMapDynamic(L, snapshot, fnArg, snapshotElem, arity, loc);
...
/** Regex method calls, both directions: `re.test(s)` on a regex receiver,
  ...
  export function lowerRegexMethodCall(L: Lowerer, call: ts.CallExpression,
    ...
    if (receiverKind === "regex" && name === "exec") {
      ...
      const resultT: IrType = { kind: "union", unionId: L.unions.intern([regexCaptureArray(L.unions), { kind: "nullT" }]) };
    ...
    if ((receiverKind === "string" || nullableStringRecv) && name === "match") {
      ...
      const exactT: IrType = { kind: "union", unionId: L.unions.intern([regexCaptureArray(L.unions), { kind: "nullT" }]) };
    ...
    if (receiverKind === "string" && name === "matchAll") {
      ...
      return {
        ...
        receiver,
        args: [re],
        type: arrayOf(regexCaptureArray(L.unions)),
        loc,
      };
    }
