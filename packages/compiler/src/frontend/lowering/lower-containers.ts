  ...
  export function lowerArrayMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (name === "push" || name === "unshift") {
      ...
      if (call.arguments.length > 1 && call.arguments.some(ts.isSpreadElement)) {
        return lowerArraySpreadInsert(lowerer, call, receiver, receiverIr, name);
      }
    ...
    if (name === "flat") return lowerArrayFlatCall(lowerer, call, access, elem, receiverIr);
...
/** Lowers and validates a HOF callback argument. The lib declares optional
  ...
  function hofCallbackArg(lowerer: Lowerer, argNode: ts.Expression, lead: IrType[], arrT: IrType):
    ...
    if (!typeEquals(fnArg.type, expected)) {
      if (!lowerer.cleanFuncAdaptable(fnArg.type, expected)) {
        lowerer.badType(argNode, lowerer.typeOf(argNode));
      }
      fnArg = lowerer.coerceInto(argNode, fnArg, expected);
    }
    return {
...
/** `a.map(fn)` / `a.filter(fn)` / `a.forEach(fn)` desugar to a direct
  ...
  function lowerArrayHofCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (method === "map" && fnRet.kind === "dyn") {
      return lowerArrayMapDynamic(lowerer, receiver, fnArg, elem, arity, loc);
    }
...
function filterResultElem(
  ...
  if (!booleanFilter) {
    ...
    // filter skips holes, so the proof ranges over stored elements only.
...
/** Read-only tuple methods snapshot fields into an array (the for-of-over-
  ...
  export function lowerTupleReadMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (name === "includes") {
      ...
      const needle = lowerer.lowerExprExpecting(call.arguments[0]!, snapshotElem);
      return {
        kind: "arrIntrinsic",
        method: "includes",
        receiver: snapshot,
    ...
    if (fnRet.kind === "dyn") return lowerArrayMapDynamic(lowerer, snapshot, fnArg, snapshotElem, arity, loc);
 ...
 * array/non-array unions and explicit depths keep the surface fence. */
  function lowerArrayFlatCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    const helper = arrayFlatHelper(lowerer, elem, loc);
 ...
 * into the fresh result, preserving flat's shallow-copy identity. */
  function arrayFlatHelper(lowerer: Lowerer, innerT: IrType & { kind: "array" }, loc: SrcLoc): string {
    const key = `flat:${typeKey(innerT)}`;
    const existing = lowerer.arrHofHelpers.get(key);
    if (existing) return existing;
    const name = `%arr.flat.${lowerer.arrHofHelpers.size}`;
    lowerer.arrHofHelpers.set(key, name);
  ...
  function buildArrayHofFn(lowerer: Lowerer, name: string,
    ...
    if (method === "map" || method === "flatMap") {
      ...
      body = [
        ...
        readLen,
        ...(method === "map" && !lowerer.nativeDenseArrays ? [{ kind: "arraySetLength", arr: varRef("out.0", outT, loc), length: varRef("n.0", F64, loc), loc } satisfies IrStmt] : []),
        forLoop([
          {
            kind: "if",
            cond: hasElem,
...
/** `a.find(f)` / `a.findIndex(f)` / `a.findLast(f)` / `a.findLastIndex(f)`
  ...
  function lowerArrayFindLikeCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    elem: IrType,): IrExpr {
    const receiver = lowerer.lowerExpr(access.expression);
    return lowerArrayFindLikeOnReceiver(lowerer, call, receiver, method, elem);
  }

  function lowerArrayFindLikeOnReceiver(lowerer: Lowerer, call: ts.CallExpression,
    receiver: IrExpr,
    method: "find" | "findIndex" | "findLast" | "findLastIndex" | "some" | "every",
    elem: IrType,): IrExpr {
    const loc = locOf(call);
  ...
  export function lowerMapMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (name === "get") {
      ...
      if (receiverIr.value.kind === "dyn") {
        if (type.kind !== "dyn") lowerer.badType(call, lowerer.typeOf(call));
...
/** Regex method calls, both directions: `re.test(s)` on a regex receiver,
  ...
  export function lowerRegexMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if ((receiverKind === "string" || nullableStringRecv) && name === "match") {
      ...
      // RegExpMatchArray | null maps to the optional-string row union by
      ...
      // itself answers a row or null, and the chain wrapper widens).
    ...
    // optional-string rows (match's rule). Lazy vs
    ...
    if (
      ...
      if (name !== "split") fenceFunctionReplacement(lowerer, call.arguments[1]);
...
/** `s.slice(1, 4)` and friends → strIntrinsic. Null when this isn't an
  ...
  export function lowerStringMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    const islandChainReceiver = dynReceiver === undefined
      ? lowerer.chainRecvByNode.get(access.expression)
      : undefined;
    ...
    if (dynReceiver === undefined) {
      const receiverIr = islandChainReceiver?.type.kind === "jsval"
        ? lowerer.mapTypeOf(lowerer.checker.getNonNullableType(lowerer.typeOf(access.expression)))
        : lowerer.mapTypeOf(lowerer.typeOf(access.expression));
    ...
    if (islandChainReceiver?.type.kind === "jsval") {
      ...
      lowerer.requireDynamicApi("'?.trim()' on an island-backed string", call);
      const result: IrExpr = {
        kind: "jsOp",
        op: "callMethod",
    ...
    if (entry.method === "replace" || entry.method === "replaceAll") {
      fenceFunctionReplacement(lowerer, call.arguments[1]);
    }
...
/** Method calls on typed-array/Buffer receivers: slice and subarray (BOTH
  ...
  export function lowerBytesMethodCall(lowerer: Lowerer, call: ts.CallExpression,
    ...
    if (name === "set") return lowerBytesSetCall(lowerer, call, access, receiverIr);
    if (name === "toString") {
