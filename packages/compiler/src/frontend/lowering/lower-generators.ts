...
import { BOOL, DYN, EFFECT_T, IrExpr, IrStmt, IrType, SrcLoc, UNDEFINED_T, VOID, isUnitType, typeEquals } from "../../ir/ir.js";
...
function isYieldableError(lowerer: Lowerer, type: IrType): boolean {
  if (type.kind === "union") return (lowerer.unions.get(type.unionId)?.arms ?? []).every((arm) => isYieldableError(lowerer, arm));
  const form = type.kind === "object" ? lowerer.classes.get(type.className)?.schema?.form : undefined;
...
export function lowerYieldStarStatement(lowerer: Lowerer, expr: ts.Expression): IrStmt | null {
  ...
  if (gen.yieldT.kind === "effect") return { kind: "exprStmt", expr: lowerYield(lowerer, expr), loc: locOf(expr) };
  if (lowerer.ctx.isAsync) {
    lowerer.unsupported(
      "SC1071",
      expr,
