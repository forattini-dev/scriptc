...
export function generateSurfaceManifest(compilerVersion: string): SurfaceManifest {
  ...
  const mathProps = new Set([
    ...Object.keys(STATIC_MATH_PROPS),
    ...Object.keys(ISLAND_SURFACE.math.props),
  ]);
  for (const name of mathProps) {
    if (STATIC_MATH_PROPS[name] !== undefined) {
      add({
        ...
        name: `Math.${name}`,
        status: "static",
        note: "folds to the ECMAScript constant's exact f64 value at compile time",
      });
    } else {
      add({ id: `stdlib.math.${name}`, kind: "stdlib", name: `Math.${name}`, status: "dynamic-only", code: "SC2012" });
    }
