...
export type IrLibFn =
  ...
  | "net.listen"
  | "net.listenCb"
  /** Explicit-interface listen: [server, port, host, ipv6Only, callback?].
   * Empty host selects dual-stack any; ipv6Only sets IPV6_V6ONLY.
   * Retained when reusePort is omitted for serialized-IR compatibility.
   * Bind failures emit async 'error' with the requested host; never throws. */
  | "net.listenOpts"
  | "net.listenOptsCb"
  /** Additive ABI: [server, port, host, ipv6Only, reusePort, callback?].
   * The extra BOOL requests kernel connection distribution. */
