...
/* ── SHA-512 / SHA-384 (FIPS 180-4) — the wider digests of the fused
 * createHash/createHmac chains. The SHA-256 shape above in 64-bit words:
 * 80 rounds, a 128-byte block, and a 128-bit big-endian length field (the
 * high half is always zero for an in-memory input). SHA-384 is SHA-512
 * under a different IV, truncated to 48 bytes. */
...
/* is384 picks the SHA-384 IV and the 48-byte truncation; the rest is
 * shared. Returns the digest length. */
static size_t scr_sha512_digest(const unsigned char *data, size_t len, unsigned char out[64],
  ...
  uint64_t h[8];
  memcpy(h, is384 ? iv384 : iv512, sizeof(h));
  size_t i = 0;
  ...
  memcpy(tail, data + i, rem);
  tail[rem] = 0x80;
  /* The 128-bit length needs 16 free bytes after the terminator. */
 ...
 * (createHash concatenates its update() chunks JS-side) AND the fused
 * static chains' dispatch. Returns the digest length, 0 for an unknown
 * algorithm. */
size_t scr_crypto_digest_raw(const char *alg, const unsigned char *data, size_t len,
  ...
  if (strcmp(alg, "sha512") == 0) return scr_sha512_digest(data, len, out, false);
  if (strcmp(alg, "sha384") == 0) return scr_sha512_digest(data, len, out, true);
  if (strcmp(alg, "md5") == 0) return scr_md5_digest(data, len, out);
  return 0;
}

/* HMAC (RFC 2104). The block size is the digest's own: 64 for md5/sha1/
 * sha256, 128 for the SHA-512 family (sha384 included — it is SHA-512
 * truncated, so it keeps the 128-byte block). */
size_t scr_crypto_hmac_raw(const char *alg, const unsigned char *key, size_t keylen,
  ...
  size_t block = (strcmp(alg, "sha512") == 0 || strcmp(alg, "sha384") == 0) ? 128 : 64;
  ...
  if (!inner) return 0;
  for (size_t i = 0; i < block; i++) inner[i] = kblock[i] ^ 0x36;
  memcpy(inner + block, data, len);
  unsigned char ih[64];
  size_t in = scr_crypto_digest_raw(alg, inner, block + len, ih);
  free(inner);
  if (in == 0) return 0;
  unsigned char outer[192];
  for (size_t i = 0; i < block; i++) outer[i] = kblock[i] ^ 0x5c;
  memcpy(outer + block, ih, in);
  return scr_crypto_digest_raw(alg, outer, block + in, out);
}

/* An ScrStr algorithm name as a NUL-terminated C string. Every lowered
 * name is a short compile-time literal ("sha1".."sha512"), so the fixed
 * buffer cannot overflow; a longer name would have been fenced. */
static void scr_digest_alg_name(const ScrStr *alg, char out[16]) {
  size_t n = alg->len < 15 ? alg->len : 15;
  memcpy(out, alg->data, n);
  out[n] = '\0';
}
...
static ScrStr *scr_hmac_digest_raw(const ScrStr *alg, const ScrBytes *key,
  ...
  char name[16];
  scr_digest_alg_name(alg, name);
  size_t n = scr_crypto_hmac_raw(name, key->data, key->len * scr_bytes_elem_size(key->elem),
                                 data, len, d);
  return scr_digest_encode(d, n, enc);
}
