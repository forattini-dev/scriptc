...
static const char web_prelude[] =
    ...
    "    if (v instanceof DOMException) { const d = new DOMException(v.message, v.name); memo.set(v, d); return d; }\n"
    ...
    "      if (tr !== undefined) {\n"
    "        const sequenceError = () => scErr(\"Failed to execute 'structuredClone': transfer in Options can not be converted to sequence.\");\n"
    "        if (tr === null || (typeof tr !== 'object' && typeof tr !== 'function')) throw sequenceError();\n"
    "        const method = tr[Symbol.iterator];\n"
    ...
    "        /* Preserve user exceptions from iterator getters, calls and steps. */\n"
    "        const iterator = method.call(tr);\n"
    "        const list = [];\n"
    "        for (;;) {\n"
    "          const next = iterator?.next;\n"
