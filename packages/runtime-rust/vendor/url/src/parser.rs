...
impl<'a> Parser<'a> {
    ...
    pub fn parse_path_start<'i>(
        ...
        if scheme_type.is_special() {
            ...
            // The authority delimiter is not a path. Always start a path,
            // even when the prefix ends in "file://", and consume its first
            // input slash exactly once. Subsequent empty segments survive.
            self.serialization.push('/');
    ...
    pub fn parse_path<'i>(
        ...
        // Preserve empty leading file-path segments, as current WHATWG URL
        // and Node do. They are significant during relative resolution.
