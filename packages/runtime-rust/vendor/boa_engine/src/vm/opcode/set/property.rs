...
fn set_by_name(
    ...
    // scriptc: see get/property.rs — a setter that redefines its own
    // property mid-call moves the object to another shape; the slot
    // recorded describes the shape before the call.
    ...
    if succeeded && slot.is_cacheable() {
        ...
        let accessor = slot.attributes.has_get() || slot.attributes.has_set();
        if !accessor || shape.to_addr_usize() == shape_before {
            ic.set(shape, slot);
        }
    }
