...
fn get_by_name<const LENGTH: bool>(
    ...
    // scriptc: the shape the slot below describes. A getter may redefine
    // its own property while it runs (a lazy property converting itself
    // into a data property), transitioning the object to another shape;
    // caching that new shape with the pre-call slot would make the next
    // read call the stored value as a getter.
    let shape_before = object.borrow().shape().to_addr_usize();
    ...
    if slot.is_cacheable() {
        ...
        if shape.to_addr_usize() == shape_before {
            ic.set(shape, slot);
        }
    }
