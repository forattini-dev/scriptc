...
use crate::function::ClassElementName;
use crate::{
    ...
    fn visit_class_element_mut(
        ...
        match node {
            ClassElement::MethodDefinition(node) => {
                // scriptc: a COMPUTED method name is an expression too —
                // TypeScript's private-field downlevel puts function
                // expressions with their own `var`s in it, and an
                // unanalyzed function body panics the bytecompiler.
                if let ClassElementName::PropertyName(name) = &mut node.name {
...
impl<'ast> VisitorMut<'ast> for BindingCollectorVisitor<'_> {
    ...
    fn visit_class_element_mut(
        ...
        match node {
            ClassElement::MethodDefinition(node) => {
                // scriptc: see the first visitor — the computed name's
                // expressions carry scopes of their own.
                if let ClassElementName::PropertyName(name) = &mut node.name {
                    self.visit_property_name_mut(name)?;
                }
                let strict = node.body.strict();
