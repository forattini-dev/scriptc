...
- **The embedded engine follows the backend.** Rust uses V8 for dynamic islands by default, with Boa available as an alternative. C and LLVM use quickjs-ng.
- **Engine-thrown error messages can differ from Node's.** The wording depends on the selected embedded engine and its version. For portable error handling, match the error type rather than engine-specific message text.
...
Direct reads of <code>import.meta.url</code>, <code>import.meta.filename</code>, and <code>import.meta.dirname</code> identify the original source module. The entry and imported modules retain distinct identities after compilation. Use <code>import.meta.main</code> to detect direct execution: comparing <code>import.meta.url</code> with <code>process.argv[1]</code> is not a portable native entry check, because the latter identifies the executable. This replaces the fork's earlier executable-valued <code>import.meta.url</code> behavior. These direct metadata reads are lowered natively in ESM, including builds that enable <code>--dynamic</code>. Bare metadata objects, computed reads, writes, and loader operations such as <code>import.meta.resolve</code> retain explicit diagnostics in native lowering.
