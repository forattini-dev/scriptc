...
3. **Backends** — `--emit=c` writes readable C and stops; `--emit=llvm` writes textual LLVM IR and stops. Neither source-output command discovers or invokes a native toolchain. On supported macOS, Linux, and Windows hosts (and for WASI), `--emit=asm|obj` sends LLVM IR to a version-matched out-of-process LLVM 22 helper; it needs no clang or linker. Executable builds default to Rust and compile the emitted safe Rust with Cargo/rustc. `--emit=rust` writes Rust source without invoking the native toolchain. C and LLVM require explicit selection, and unsupported constructs produce diagnostics without switching backends. WASI requires the explicit LLVM backend.
