...
- **Cargo and rustc** — Rust is the primary and default native backend.
- **Node ≥ 24** — to run the compiler. The binaries it produces need no Node at all.
- **clang** — required when explicitly selecting the C/LLVM backends; see [platform support](/platforms).
