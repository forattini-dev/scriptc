// @target node26
// read() boundaries under --target node26: the twin of 2845 pinned to the
// matrix CANDIDATE. Node 26 (nodejs#60441) makes howMuchToRead(NaN)
// `!state.decoder ? state.buffer.first().length : state.length`, so a
// PAUSED bare read() hands back the head chunk instead of collapsing the
// queue; read(n) slices across boundaries exactly as before. The oracle
// is the pinned Node 26 interpreter, byte for byte; the binary carries
// the switch through runtime::target_configure (target_config.rs).
