...
/* ── the backend column ──────────────────────────────────────────────────
 * `backends` answers "which of the three code-generating backends lowers
 * this?" — the artifact the Rust-replaces-C migration is read against. It
 * is derived, never written, so what these tests hold is the derivation's
 * shape: the generated table underneath still spans the IR's whole libCall
 * universe, and no entry can quietly publish an empty or invented column.
 */
describe("surface manifest backend column", () => {
  test("the generated backend table spans exactly the IrLibFn universe", () => {
    // BACKEND_LIB_CALLS is generated by scripts/gen-backend-libcalls.mjs
    // from LIB_FN_SIGS. If a libCall spelling is added to the IR and the
    // table is not regenerated, the column silently stops seeing it — so
    // the two key sets must agree exactly.
    const universe = Object.keys(LIB_FN_SIGS).sort();
    const tabled = Object.keys(BACKEND_LIB_CALLS);
    expect(
      tabled,
      "packages/compiler/src/coverage/backend-libcalls.ts is stale — run 'pnpm gen:backend-libcalls'",
    ).toEqual(universe);
    for (const [fn, ids] of Object.entries(BACKEND_LIB_CALLS)) {
      // A spelling no backend lowers would mean the IR can carry a call
      // nothing emits — the generator refuses to write that, and so does
      // this test if the file is edited by hand.
      expect(ids.length, `${fn} is lowered by no backend`).toBeGreaterThan(0);
      expect([...ids], `${fn}: backend ids must be valid and in column order`).toEqual(
        BACKEND_IDS.filter((backend) => ids.includes(backend)),
      );
    }
  });

  test("every entry carries a column: 'unknown', or a non-empty ordered subset of the backends", () => {
    for (const e of manifest.entries) {
      expect(e.backends, `${e.id}: every entry must carry a backend column`).toBeDefined();
      if (e.backends === "unknown") continue;
      expect(Array.isArray(e.backends), `${e.id}: a derived column is a list`).toBe(true);
      // The requested invariant: never empty without saying "unknown".
      // An empty derived list would read as "no backend lowers this",
      // which for a surface projected FROM a lowering table is a tree
      // inconsistency, not a publishable claim.
      expect(e.backends.length, `${e.id}: derived column is empty — say 'unknown' instead`).toBeGreaterThan(0);
      expect([...e.backends], `${e.id}: column must be a subset of ${BACKEND_IDS.join(",")} in that order`).toEqual(
        BACKEND_IDS.filter((backend) => e.backends.includes(backend)),
      );
    }
  });

  test("rust is a subset of what the backends collectively lower, and the column is populated", () => {
    const derived = manifest.entries.filter((e) => e.backends !== "unknown");
    // The projection must actually reach a meaningful slice: an all-unknown
    // column would satisfy every shape check above while saying nothing.
    expect(derived.length, "no entry has a derived backend column").toBeGreaterThan(50);
    const rust = derived.filter((e) => e.backends.includes("rust"));
    expect(rust.length, "no entry is lowered by the rust backend").toBeGreaterThan(0);
    // rust ⊆ the union: an entry rust lowers is, by construction, an entry
    // the union of backends lowers — which is exactly the non-empty
    // derived column asserted above. Stated here as the migration reading
    // it: every rust-covered entry is a covered entry.
    for (const e of rust) {
      expect(e.backends.length, `${e.id}`).toBeGreaterThan(0);
    }
  });

  test("only entries with a libCall linkage carry a derived column", () => {
    // The derivation's one rule: a column is derived from the libCall
    // spellings a lowering table gives the entry. Every such table row is
    // a supported lowering, so a derived column implies a static status —
    // and a non-static entry (a refusal, or an island-only surface) has no
    // lowering to attribute and must say "unknown".
    for (const e of manifest.entries) {
      if (e.backends === "unknown") continue;
      expect(e.status, `${e.id}: a derived backend column implies a static entry`).toBe("static");
    }
  });
});
