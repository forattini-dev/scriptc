...
const entry = join(import.meta.dirname, "../fixtures/npm-static/optional-index-cli.ts");
const sanitize = process.env["SCRIPTC_SAN"] === "1";
for (const backend of ["rust", "c", "llvm"] as const) {
  test.skipIf(sanitize && backend === "rust")(`${backend} preserves absent indexes through npm implicit specialization`, async () => {
    const outDir = mkdtempSync(join(tmpdir(), `scriptc-npm-index-${backend}-`));
    try {
