...
// header): the server program's stdout/exit code must match ONE Node's
// fixed behavior, so this pins to the compat matrix primary rather than
// whichever `node` the PATH happens to resolve. The client driver spawned
// inside runLane stays plain "node" — it is the identical fixed workload
// on both lanes, not itself an oracle comparison subject.
