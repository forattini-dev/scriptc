...
test.skipIf(sanitize)("Rust dynamic fetch accepts a constructed Request", async () => {
  const entry = join(fixturesRoot, "fetch-request-defaults/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic fetch applies a constructed RequestInit", async () => {
  const entry = join(fixturesRoot, "fetch-request-init/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic fetch rejects a pre-aborted signal", async () => {
  const entry = join(fixturesRoot, "fetch-abort-pre/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic AbortController rejects fetch after abort", async () => {
  const entry = join(fixturesRoot, "fetch-abort-controller-pre/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic AbortSignal.timeout aborts asynchronously", async () => {
  const entry = join(fixturesRoot, "fetch-abort-timeout/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic fetch rejects when timeout fires in flight", async () => {
  const entry = join(fixturesRoot, "fetch-abort-in-flight/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic fetch inherits a Request dependent signal", async () => {
  const entry = join(fixturesRoot, "fetch-request-signal/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry, baseUrl]),
    runBinary(binary, [baseUrl], {
...
test.skipIf(sanitize)("Rust dynamic Request instanceof uses realm identity", async () => {
  const entry = join(fixturesRoot, "fetch-request-instanceof/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
...
test.skipIf(sanitize)("Rust dynamic Request.text consumes a textual body", async () => {
  const entry = join(fixturesRoot, "fetch-request-text/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
...
test.skipIf(sanitize)("Rust dynamic Request.bytes returns exact UTF-8 bytes", async () => {
  const entry = join(fixturesRoot, "fetch-request-bytes/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
...
test.skipIf(sanitize)("Rust dynamic Request.arrayBuffer exposes the body byte length", async () => {
  const entry = join(fixturesRoot, "fetch-request-array-buffer/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
  ...
  expect(nodeRes.exitCode, nodeRes.stderr.toString("utf8")).toBe(0);
  expect(nativeRes.exitCode, nativeRes.stderr.toString("utf8")).toBe(nodeRes.exitCode);
  expect(nativeRes.stdout.toString("utf8"), nativeRes.stderr.toString("utf8"))
    .toBe(nodeRes.stdout.toString("utf8"));
}, 120_000);

test.skipIf(sanitize)("Rust dynamic Request.body exposes a lazy readable stream", async () => {
  const entry = join(fixturesRoot, "fetch-request-body/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
...
test.skipIf(sanitize)("Rust dynamic Request.body reads disturb the Body state", async () => {
  const entry = join(fixturesRoot, "fetch-request-body-read/main.ts");
  const binary = await build(entry, "rust", "dev");
  const [nodeRes, nativeRes] = await Promise.all([
    runBinary(oracleExecutable, [entry]),
    runBinary(binary, [], {
