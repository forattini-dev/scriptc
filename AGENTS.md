...
These rules apply repository-wide, including `internal/compatibility/`. The docs site also follows `docs/AGENTS.md`.
...
- `internal/compatibility/dynamic-support.json` defines dynamic-island modules, exports, members, explicit stubs, globals, and policies. Its `runtimeRegistry` configuration references the full bootstrap registration and export tables, with explicit engine-specific overrides, to generate the runtime builtin registry. Keep linkage broader than the test-backed compatibility inventory; missing evidence must not remove existing runtime exports.
